...
export default function PropertyForm({ existingHomes = [], staff = [], onSubmit, onClose, saving }) {
  ...
  const validate = () => {
    const errs = {};
    if (mode === "new") {
      if (!data.name?.trim()) errs.name = "Home name is required";
      if (!data.address?.trim()) errs.address = "Address is required";
      if (!data.team_leader_id) errs.team_leader_id = "Please select a team leader";
      if (data.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.email)) errs.email = "Invalid email address";
      if (data.postcode && !/^[A-Z]{1,2}[0-9][0-9A-Z]?\s?[0-9][A-Z]{2}$/i.test(data.postcode)) errs.postcode = "Invalid UK postcode";
    }
    return errs;
  };

  const handleSubmit = () => {
    if (mode === "existing" && !selectedHomeId) return;
    const errs = validate();
  ...
  const canSave = mode === "existing" ? !!selectedHomeId : true;
          ...
          {(mode === "new" || selectedHomeId) && (
            <>
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <Label>Name *</Label>
                  <Input className={`mt-1.5 ${errors.name ? "border-destructive" : ""}`} value={data.name} onChange={e => set("name", e.target.value)} placeholder="e.g. Maple House" />
              ...
              <div>
                <Label>Address *</Label>
                <Input className={`mt-1.5 ${errors.address ? "border-destructive" : ""}`} value={data.address} onChange={e => set("address", e.target.value)} />
                {errors.address && <p className="text-xs text-destructive mt-1">{errors.address}</p>}
              </div>
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <Label>Postcode</Label>
                  <Input className={`mt-1.5 ${errors.postcode ? "border-destructive" : ""}`} value={data.postcode} onChange={e => set("postcode", e.target.value.toUpperCase())} />
              ...
              <div>
                <Label>Email</Label>
                <Input className={`mt-1.5 ${errors.email ? "border-destructive" : ""}`} type="email" value={data.email} onChange={e => set("email", e.target.value)} />
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <Select value={data.team_leader_id} onValueChange={v => set("team_leader_id", v)}>
                    <SelectTrigger className={`mt-1.5 ${errors.team_leader_id ? "border-destructive" : ""}`}><SelectValue placeholder="Select team leader…" /></SelectTrigger>
                    <SelectContent>
