...
import { ArrowLeft, Wand2, AlertTriangle, CheckCircle2, Users, ChevronDown, X } from "lucide-react";
...
export default function RotaGenerateFlow({ home, user, myStaffProfile, onBack, onComplete }) {
  ...
  const companyStaff = allStaff; // Show all company staff, not just home-assigned
  const includedStaff = companyStaff.filter(s => !excludedStaff.includes(s.id));
      ...
      <div className="bg-card border border-border rounded-xl p-4 space-y-3">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          ...
          {companyStaff.length > 0 && (
            <div className="flex gap-1 flex-wrap">
              <button
                onClick={() => {
                  const supportWorkers = companyStaff.filter(s => s.role === "support_worker").map(s => s.id);
                  setExcludedStaff(prev => {
                    const filtered = prev.filter(id => !supportWorkers.includes(id));
                    return supportWorkers.every(id => !filtered.includes(id)) ? prev : filtered;
                  });
                }}
              ...
              <button
                onClick={() => {
                  const bankStaff = companyStaff.filter(s => s.role === "bank_staff").map(s => s.id);
                  setExcludedStaff(prev => {
                    const filtered = prev.filter(id => !bankStaff.includes(id));
                    return bankStaff.every(id => !filtered.includes(id)) ? prev : filtered;
                  });
                }}
              ...
              <button
                onClick={() => {
                  const teamLeaders = companyStaff.filter(s => s.role === "team_leader").map(s => s.id);
                  setExcludedStaff(prev => {
                    const filtered = prev.filter(id => !teamLeaders.includes(id));
                    return teamLeaders.every(id => !filtered.includes(id)) ? prev : filtered;
                  });
                }}
              ...
              <button
                onClick={() => {
                  const teamManagers = companyStaff.filter(s => s.role === "team_manager").map(s => s.id);
                  setExcludedStaff(prev => {
                    const filtered = prev.filter(id => !teamManagers.includes(id));
                    return teamManagers.every(id => !filtered.includes(id)) ? prev : filtered;
                  });
                }}
        ...
        <div className="relative">
          <button
            ...
            className="w-full flex items-center justify-between p-3 border border-border rounded-lg bg-card hover:bg-muted/30 transition-colors text-left"
          >
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <Users className="w-4 h-4 text-muted-foreground shrink-0" />
              {includedStaff.length === 0 ? (
                <span className="text-sm text-muted-foreground">Select staff members...</span>
              ) : includedStaff.length === companyStaff.length ? (
                <span className="text-sm font-medium">All staff selected</span>
              ) : (
                <div className="flex gap-1 flex-wrap">
                  {includedStaff.slice(0, 2).map(s => (
                    <span key={s.id} className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded">
                      {s.full_name.split(" ")[0]}
                    </span>
                  ))}
                  {includedStaff.length > 2 && (
                    <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded">
                      +{includedStaff.length - 2} more
                    </span>
                  )}
                </div>
              )}
            </div>
            <ChevronDown className={`w-4 h-4 text-muted-foreground shrink-0 transition-transform ${staffSearchOpen ? "rotate-180" : ""}`} />
          </button>

          {staffSearchOpen && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-card border border-border rounded-lg shadow-lg z-50 max-h-96 overflow-hidden flex flex-col">
              <Input
                placeholder="Search staff..."
                value={staffSearch}
                onChange={e => setStaffSearch(e.target.value)}
                className="h-9 border-0 border-b rounded-none text-sm"
                autoFocus
              />
              <div className="overflow-y-auto flex-1">
                {companyStaff
                  .filter(s => s.full_name.toLowerCase().includes(staffSearch.toLowerCase()) || s.role?.replace("_", " ").toLowerCase().includes(staffSearch.toLowerCase()))
                  .map(s => {
                    const profile = avProfiles.find(p => p.staff_id === s.id);
                    ...
                    return (
                      <label
                        key={s.id}
                        className="flex items-center gap-2 p-2.5 hover:bg-muted/50 cursor-pointer border-b border-border/30 last:border-0"
                      >
                        <input
                          type="checkbox"
                          checked={included}
                        ...
                        <div className="flex gap-1 shrink-0">
                          {!profile && <span className="text-xs text-amber-600 bg-amber-50 px-1 py-0.5 rounded">No profile</span>}
                          {profile?.sleep_in_qualified && <span className="text-xs text-blue-600 bg-blue-50 px-1 py-0.5 rounded">Sleep-in</span>}
                        </div>
                      </label>
                    );
                  })}
                {companyStaff.filter(s => s.full_name.toLowerCase().includes(staffSearch.toLowerCase())).length === 0 && (
