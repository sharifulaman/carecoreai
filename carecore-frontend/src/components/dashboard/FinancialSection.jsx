...
export default function FinancialSection({ homes, placements, invoices, bills, homeExpenses = [], pettyCashTx, isLoading }) {
  const navigate = useNavigate();
  const lastMonth = getMonthStr(-1);

  // Chart 1A — Top 5 Revenue (last month)
  const revenueData = useMemo(() => {
    if (!homes || !invoices) return [];
    return homes
      .map(home => {
        const received = (invoices || [])
          .filter(inv => inv.home_id === home.id && inv.status === "paid")
          .reduce((s, inv) => s + (inv.total_amount || 0), 0);
        return { id: home.id, name: home.name, type: home.type, received };
      })
  ...
  const netData = useMemo(() => {
    if (!homes || !invoices || !bills) return [];
    const mapped = homes
      .map(home => {
        const received = invoices
        ...
        const homeExpenseAmount = (homeExpenses || [])
          .filter(e => e.home_id === home.id && (e.date || "").startsWith(lastMonth))
        ...
        const totalExpenses = billExpenses + homeExpenseAmount + pettyOut;
    ...
    return mapped;
  }, [homes, invoices, bills, homeExpenses, pettyCashTx, lastMonth]);

  // Chart 1C — Last 6 months of financial data
  const sixMonthData = useMemo(() => {
    const months = [];
    for (let i = 5; i >= 0; i--) {
      const monthStr = getMonthStr(-i);
      ...
      const billExp = (bills || [])
        ...
        .reduce((s, b) => s + (b.amount || 0), 0);
      const homeExp = (homeExpenses || [])
        .filter(e => (e.date || "").startsWith(monthStr))
        .reduce((s, e) => s + (e.amount || 0), 0);
      const pettyExp = (pettyCashTx || [])
        .filter(tx => tx.transaction_type === "cash_out" && (tx.date || "").startsWith(monthStr))
        .reduce((s, tx) => s + (tx.amount || 0), 0);
      const expenses = billExp + homeExp + pettyExp;
      months.push({ label: monthLabel, income, expenses, net: income - expenses });
    }
    return months;
  ...
  const hasData = (homes && homes.length > 0) || (invoices && invoices.length > 0) || (bills && bills.length > 0);
