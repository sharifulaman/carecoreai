...
export default function IncidentsSection({ accidentReports, homeChecks, dailyLogs, maintenanceLogs, residents, homes }) {
  ...
  const recentIncidents = useMemo(() => {
    return [...accidentReports]
      .sort((a, b) => (b.incident_datetime || "").localeCompare(a.incident_datetime || ""))
      .slice(0, 10);
  }, [accidentReports]);
                ...
                <tbody>
                  {recentIncidents.map(a => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-3 py-2 whitespace-nowrap">{formatDateUK(a.incident_datetime)}</td>
                        <td className="px-3 py-2">
                          <span className={`px-1.5 py-0.5 rounded text-xs ${INCIDENT_TYPE_STYLES[a.incident_type] || "bg-muted text-muted-foreground"}`}>
                            {INCIDENT_TYPE_LABELS[a.incident_type] || a.incident_type}
                          </span>
                        </td>
                        <td className="px-3 py-2">
                          {pt ? (
                        ...
                        <td className="px-3 py-2">
                          <span className={`px-1.5 py-0.5 rounded capitalize ${INCIDENT_STATUS_STYLES[a.manager_review_status] || "bg-muted text-muted-foreground"}`}>
