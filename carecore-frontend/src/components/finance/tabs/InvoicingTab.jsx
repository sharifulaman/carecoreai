...
import { Plus, Eye, CheckCircle, Download, X, FileText, Lock } from "lucide-react";
...
export default function InvoicingTab({ invoices, placements, homes, residents, visibleHomes, visibleHomeIds, visibleInvoices, isAdmin, isSW, isFinanceOfficer, isFinanceManager }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {!isSW && (
          <div title={isFinanceOfficer ? "Requires Finance Manager approval" : undefined}>
            <Button size="sm" onClick={() => !isFinanceOfficer && setShowGenerate(true)} disabled={isFinanceOfficer} className="gap-1">
              {isFinanceOfficer && <Lock className="w-3.5 h-3.5" />}
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      {(inv.status === "sent" || inv.status === "overdue") && (
                        <div title={isFinanceOfficer ? "Requires Finance Manager approval" : "Mark as paid"}>
                          <button
                            onClick={() => { if (!isFinanceOfficer) { setMarkingPaid(inv); setPaidForm({ paid_date: new Date().toISOString().split("T")[0], paid_amount: String(inv.total_amount), notes: "" }); } }}
                            disabled={isFinanceOfficer}
                            className={`flex items-center gap-1 text-xs ${isFinanceOfficer ? "text-slate-300 cursor-not-allowed" : "text-green-600 hover:underline"}`}
                          >
                            {isFinanceOfficer ? <Lock className="w-3.5 h-3.5" /> : <CheckCircle className="w-3.5 h-3.5" />} Paid
                          </button>
                        </div>
                      )}
                      {inv.status === "draft" && (
                        <div title={isFinanceOfficer ? "Requires Finance Manager approval" : "Mark as sent"}>
                          <button
                            onClick={async () => { if (!isFinanceOfficer) { await secureGateway.update("PlacementInvoice", inv.id, { status: "sent", sent_date: new Date().toISOString().split("T")[0] }); qc.invalidateQueries({ queryKey: ["invoices"] }); toast.success("Marked as sent"); } }}
                            disabled={isFinanceOfficer}
                            className={`text-xs ${isFinanceOfficer ? "text-slate-300 cursor-not-allowed" : "text-blue-600 hover:underline"}`}
                          >
                            {isFinanceOfficer && <Lock className="w-3 h-3 inline mr-0.5" />}Send
                          </button>
                        </div>
                      )}
                    </div>
