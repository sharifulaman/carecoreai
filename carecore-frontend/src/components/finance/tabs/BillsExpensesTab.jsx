...
import { Plus, X, CheckCircle, Lock } from "lucide-react";
import { toast } from "sonner";
import BillApprovalBadge from "@/components/house/BillApprovalBadge";
...
export default function BillsExpensesTab({ homes, bills: propBills, expenses: propExpenses, visibleHomes, visibleHomeIds, isAdmin, isSW, isFinanceOfficer, isFinanceManager }) {
  ...
  const [myStaffProfile, setMyStaffProfile] = useState(null);

  useEffect(() => {
    const fetchProfile = async () => {
      const user = await base44.auth.me();
      if (user?.email) {
        const profiles = await secureGateway.filter("StaffProfile", { email: user.email });
        if (profiles.length > 0) setMyStaffProfile(profiles[0]);
      }
    };
    fetchProfile();
  }, []);
  ...
  const { data: workflows = [] } = useQuery({ queryKey: ["approval-workflows"], queryFn: () => secureGateway.filter("ApprovalWorkflow"), staleTime: 30 * 1000 });
  ...
  const handleAddBill = async () => {
    ...
    if (!myStaffProfile) { toast.error("Staff profile not loaded"); return; }
    setSaving(true);
    try {
      ...
      // Create bill
      const bill = await secureGateway.create("Bill", billData);
      ...
      const workflow = await secureGateway.create("ApprovalWorkflow", {
        ...
        entity_type: "bill",
        entity_id: bill.id,
        entity_reference: `Bill — ${bill.supplier} — £${bill.amount}`,
        home_id: bill.home_id,
        home_name: bill.home_name,
        amount: bill.amount,
        status: "pending_tl",
        current_step: 1,
        ...
        priority: bill.amount > 500 ? "high" : "normal",
      });
      
      // Notify team leader
      const tlProfile = homes.find(h => h.id === bill.home_id)?.team_leader_id;
      if (tlProfile) {
        await secureGateway.create("Notification", {
          org_id: ORG_ID,
          recipient_staff_id: tlProfile,
          title: "Bill Awaiting Approval",
          body: `New bill submitted for approval: ${bill.supplier} £${bill.amount.toFixed(2)}`,
          type: "approval",
          link: "/approvals",
          read: false,
      ...
      toast.success("Bill submitted for TL approval");
    } catch (err) {
                      ...
                      <tr key={b.id} className="border-b border-border/50 last:border-0 hover:bg-muted/20">
                         ...
                         <td className="px-4 py-3">
                           <div className="flex flex-col gap-1">
                             <span className={`text-xs px-2 py-0.5 rounded-full inline-block ${st?.cls || "bg-slate-100 text-slate-600"}`}>{b.status === "paid" ? "Paid" : st?.label || b.status}</span>
                             {(() => {
                               const wf = workflows.find(w => w.entity_id === b.id && w.entity_type === "bill");
                               if (wf && wf.status !== "approved" && wf.status !== "rejected") {
                                 return <span className="text-xs px-2 py-0.5 rounded-full inline-block bg-amber-100 text-amber-700">🔒 Awaiting TL Approval</span>;
                               }
                               return b.approval_status && b.approval_status !== 'draft' ? <BillApprovalBadge status={b.approval_status} /> : null;
                             })()}
                           </div>
                         </td>
                         <td className="px-4 py-3">
                           {b.status !== "paid" && (() => {
                             const wf = workflows.find(w => w.entity_id === b.id && w.entity_type === "bill");
                             const pendingApproval = wf && wf.status !== "approved" && wf.status !== "rejected";
                             const approvalOk = !pendingApproval && (!b.approval_status || b.approval_status === 'approved' || b.approval_status === 'draft');
                             const canPay = approvalOk && !isFinanceOfficer;
                             const tipText = pendingApproval ? "Approval required" : isFinanceOfficer ? "Requires Finance Manager approval" : !approvalOk ? "Awaiting approval" : "Mark as paid";
                             return (
                               <button
                                 onClick={() => canPay && markBillPaid(b)}
                                 disabled={!canPay}
                                 className={`flex items-center gap-1 text-xs ${canPay ? "text-green-600 hover:underline" : "text-slate-300 cursor-not-allowed"}`}
                                 title={tipText}
                               >
                                 {!canPay ? <Lock className="w-3.5 h-3.5" /> : <CheckCircle className="w-3.5 h-3.5" />} Mark Paid
                               </button>
                             );
                           })()}
                         </td>
