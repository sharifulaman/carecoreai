...
export default function AllowancesTab({ residents, homes, visibleHomes, visibleHomeIds, isSW }) {
  ...
  return (
    <div className="space-y-5">
      {showAddAllowance && (
        <AddAllowanceModal
          residents={residents.filter(r => visibleHomeIds.has(r.home_id))}
          homes={visibleHomes}
          onClose={() => setShowAddAllowance(false)}
          onSaved={() => { qc.invalidateQueries({ queryKey: ["allowances"] }); setShowAddAllowance(false); }}
        />
      )}
      {showAddSavings && (
        <AddSavingsModal
          ...
          onSaved={() => { qc.invalidateQueries({ queryKey: ["savings"] }); setShowAddSavings(false); }}
        />
      )}
      {/* Section toggle */}
...
function AddAllowanceModal({ residents, homes, onClose, onSaved }) {
  const { ORG_ID } = { ORG_ID: "default_org" };
  const [form, setForm] = useState({ resident_id: "", home_id: "", weekly_amount: "", payment_day: "Friday", status: "active", notes: "" });
  const [saving, setSaving] = useState(false);
  const handleSave = async () => {
    if (!form.resident_id || !form.home_id || !form.weekly_amount) { toast.error("Resident, home and amount are required"); return; }
    setSaving(true);
    await secureGateway.create("ResidentAllowance", {
      org_id: "default_org",
      resident_id: form.resident_id,
      home_id: form.home_id,
      weekly_amount: parseFloat(form.weekly_amount) || 0,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-card rounded-2xl border border-border p-6 w-full max-w-md mx-4 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold">Add Resident Allowance</h3>
          <button onClick={onClose}><X className="w-4 h-4 text-muted-foreground" /></button>
        </div>
        <div>
          <label className="text-xs font-medium">Resident *</label>
          <select value={form.resident_id} onChange={e => {
            const res = residents.find(r => r.id === e.target.value);
            setForm(f => ({ ...f, resident_id: e.target.value, home_id: res?.home_id || f.home_id }));
          }} className="mt-1 w-full text-sm border border-input rounded-md px-3 py-2 bg-transparent focus:outline-none focus:ring-1 focus:ring-ring">
        ...
        <div>
          ...
          <select value={form.home_id} onChange={e => setForm(f => ({ ...f, home_id: e.target.value }))}
            className="mt-1 w-full text-sm border border-input rounded-md px-3 py-2 bg-transparent focus:outline-none focus:ring-1 focus:ring-ring">
            <option value="">Select home…</option>
            {homes.map(h => <option key={h.id} value={h.id}>{h.name}</option>)}
          </select>
        </div>
        <div><label className="text-xs font-medium">Weekly Amount (£) *</label><Input type="number" step="0.01" value={form.weekly_amount} onChange={e => setForm(f => ({ ...f, weekly_amount: e.target.value }))} className="mt-1" placeholder="e.g. 60" /></div>
        <div>
          <label className="text-xs font-medium">Payment Day</label>
          <select value={form.payment_day} onChange={e => setForm(f => ({ ...f, payment_day: e.target.value }))}
            className="mt-1 w-full text-sm border border-input rounded-md px-3 py-2 bg-transparent focus:outline-none focus:ring-1 focus:ring-ring">
        ...
        <div><label className="text-xs font-medium">Notes</label><Input value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} className="mt-1" /></div>
        <div className="flex gap-2">
          <Button className="flex-1" onClick={handleSave} disabled={saving}>{saving ? "Saving…" : "Add Allowance"}</Button>
          <Button variant="outline" onClick={onClose}>Cancel</Button>
        </div>
      </div>
    </div>
...
function AddSavingsModal({ residents, homes, onClose, onSaved }) {
  const [form, setForm] = useState({ resident_id: "", home_id: "", current_balance: "0", notes: "" });
  const [saving, setSaving] = useState(false);
  const handleSave = async () => {
    if (!form.resident_id || !form.home_id) { toast.error("Resident and home are required"); return; }
    setSaving(true);
    await secureGateway.create("ResidentSavings", {
      org_id: "default_org",
      resident_id: form.resident_id,
      home_id: form.home_id,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-card rounded-2xl border border-border p-6 w-full max-w-md mx-4 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold">Add Savings Account</h3>
          <button onClick={onClose}><X className="w-4 h-4 text-muted-foreground" /></button>
        </div>
        <div>
          <label className="text-xs font-medium">Resident *</label>
        ...
        <div><label className="text-xs font-medium">Opening Balance (£)</label><Input type="number" step="0.01" value={form.current_balance} onChange={e => setForm(f => ({ ...f, current_balance: e.target.value }))} className="mt-1" /></div>
        <div><label className="text-xs font-medium">Notes</label><Input value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} className="mt-1" /></div>
        <div className="flex gap-2">
          <Button className="flex-1" onClick={handleSave} disabled={saving}>{saving ? "Saving…" : "Create Account"}</Button>
          <Button variant="outline" onClick={onClose}>Cancel</Button>
        </div>
      </div>
    </div>
