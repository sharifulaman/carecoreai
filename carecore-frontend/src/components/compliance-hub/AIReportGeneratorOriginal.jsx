...
export default function AIReportGenerator({
  ...
  const buildContext = (section) => {
    ...
    const ypViewsContext = ypViewsMethod ? `Young people's views gathered: ${ypViewsMethod} (${ypViewsCount} young people). Summary: ${ypViewsSummary}. Actions taken: ${ypViewsActions}` : "";
    return `
...
Write a Regulation 32 Quality of Support Review narrative for the "${section}" section only. Use UK care sector language. Be evidence-based.`;
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-purple-200" style={{ background: "#EEEDFE" }}>
      ...
      {/* Young People's Views Section */}
      <div className="p-5 mx-4 mt-4 bg-card border border-border rounded-xl space-y-4">
        <h3 className="font-semibold text-sm">Young People's Views (Required for Reg 32)</h3>
        <p className="text-xs text-muted-foreground">
          Ofsted requires evidence that young people's views have been sought and acted upon.
        </p>

        <div className="space-y-4">
          <div>
            <label className="text-xs font-semibold">How were young people's views gathered?</label>
            <select
              value={ypViewsMethod}
              onChange={e => setYpViewsMethod(e.target.value)}
              className="w-full mt-1 border border-border rounded-lg px-3 py-2 text-sm bg-background"
            >
              <option value="">Select method...</option>
              <option value="individual_1to1">Individual 1:1 conversations</option>
              <option value="group_meeting">Group meeting / house meeting</option>
              <option value="written_survey">Written survey / questionnaire</option>
              <option value="key_worker_records">Key worker records reviewed</option>
              <option value="complaints_advocacy">Complaints and advocacy records</option>
              <option value="mixed">Mixed methods</option>
            </select>
          </div>

          <div>
            <label className="text-xs font-semibold">Number of young people whose views were gathered</label>
            <input
              type="number"
              value={ypViewsCount}
              onChange={e => setYpViewsCount(e.target.value)}
              className="w-full mt-1 border border-border rounded-lg px-3 py-2 text-sm bg-background"
              min="0"
            />
          </div>
          ...
          <div>
            <label className="text-xs font-semibold">Summary of what young people said</label>
            <textarea
              value={ypViewsSummary}
              onChange={e => setYpViewsSummary(e.target.value)}
              rows={4}
              placeholder="e.g. Young people reported feeling safe and supported. Three young people raised concerns about meal variety. All young people were aware of how to make a complaint..."
              className="w-full mt-1 border border-border rounded-lg px-3 py-2 text-sm bg-background resize-none"
            />
          </div>

          <div>
            <label className="text-xs font-semibold">What changes were made as a result?</label>
            <textarea
              value={ypViewsActions}
              onChange={e => setYpViewsActions(e.target.value)}
              rows={3}
              placeholder="e.g. Menu was reviewed and updated. Key workers addressed concerns in individual sessions..."
              className="w-full mt-1 border border-border rounded-lg px-3 py-2 text-sm bg-background resize-none"
            />
          </div>
        </div>
