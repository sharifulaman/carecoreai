...
export default function Reg20SafeguardingPolicy({ staffProfile }) {
  const qc = useQueryClient();
  const policyType = "safeguarding";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold">Regulation 20 — Safeguarding Policy</h2>
          <p className="text-sm text-muted-foreground mt-0.5 max-w-2xl">
            A written safeguarding policy must be in place covering protection from abuse or neglect and the procedure to follow when an allegation is made.
          </p>
        </div>
      ...
      {!activePlan && !pendingPlan && (
        <div className="bg-red-50 border border-red-300 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-red-700">No safeguarding policy exists. This is required for Ofsted registration.</p>
            {canEdit && (
      ...
      {activePlan && isOverdue && (
        <div className="bg-amber-50 border border-amber-300 rounded-xl p-4">
          <p className="text-sm font-semibold text-amber-700">Your safeguarding policy is overdue for review (due {format(new Date(activePlan.review_date), "dd MMM yyyy")}).</p>
        </div>
      )}
      ...
      {activePlan && !isOverdue && !isReviewSoon && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4">
          <p className="text-sm font-medium text-green-700">Safeguarding policy is current. Version {activePlan.version_number} effective {format(new Date(activePlan.effective_date), "dd MMM yyyy")}.</p>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          {!isLoading && !activePlan && !pendingPlan && (
            <div className="bg-card border border-border rounded-xl p-12 text-center text-muted-foreground text-sm">
              No active safeguarding policy. {canEdit && <button className="text-primary hover:underline ml-1" onClick={() => setShowForm(true)}>Create one now →</button>}
            </div>
          )}
