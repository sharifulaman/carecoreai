...
export default function OffboardingModal({ member, org, onClose, onComplete }) {
  ...
  const generateLetter = async () => {
    ...
    try {
      // Use a dummy template instead of an LLM call since the integration might not be available
      const template = `[Organisation Name]
...
I am writing to formally acknowledge and accept your letter of resignation from the position of ${member.job_title || member.role?.replace(/_/g, " ")} at ${org?.name || "CareCore"}, which we received on ${format(parseISO(resignDate), 'dd/MM/yyyy')}.

As per your notice period of ${noticePeriodWeeks} weeks, we confirm that your final working day will be ${format(parseISO(lastWorkingDay), 'dd/MM/yyyy')}.

Your final pay, including any outstanding holiday entitlement, will be processed in the next available payroll run following your departure. Your P45 will be issued shortly thereafter. 

We would like to take this opportunity to thank you for your contributions during your time with us and wish you the very best in your future endeavours.
  ...
  const confirmOffboarding = useMutation({
    mutationFn: async () => {
      ...
      await secureGateway.create("AuditTrail", {
        ...
        new_value: { status: "left", end_date: lastWorkingDay, checklist, letter },
      }).catch(() => {});
    },
    onSuccess: () => {
