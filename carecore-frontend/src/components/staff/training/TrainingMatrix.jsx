...
export default function TrainingMatrix({ filteredStaff, activeCourses, recordMap, recordMapByName, requirements, staffProfile, homes = [], panelFilters = {} }) {
  const homeMap = Object.fromEntries(homes.map(h => [h.id, h]));
  const [search, setSearch] = useState("");
  const [page, setPage] = useState(0);
  const [editCell, setEditCell] = useState(null);

  const { isReadOnly } = useModuleActions("staff", { isReadOnly: staffProfile?.role === "support_worker" });
  ...
  const exportCSV = () => {
    ...
    const rows = filteredStaff.map(s => {
      ...
      const cells = activeCourses.map(c => {
        const rec = recordMapByName[`${s.id}__${c.course_name}`];
