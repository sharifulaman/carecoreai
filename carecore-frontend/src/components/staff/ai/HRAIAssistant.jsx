...
function buildContext({ staff, residents, homes, incidents, supervisions, training, balances, leaves, disciplinary, quizResults, policyAssignments, expenses, wellbeing }) {
  const today = new Date().toISOString().split("T")[0];
  return `
TODAY: ${today}

ACTIVE HOMES (${homes.filter(h => h.status === "active").length} total):
${homes.filter(h => h.status === "active").map(h =>
  `- ${h.name} | Type: ${h.type} | Address: ${h.address || "N/A"} | Team Leader: ${h.team_leader_id || "N/A"} | Status: ${h.status}`
).join("\n") || "No homes."}

ACTIVE RESIDENTS (${residents.filter(r => r.status === "active").length} total):
${residents.filter(r => r.status === "active").slice(0, 30).map(r =>
  `- ${r.display_name} | Age: ${r.dob ? new Date().getFullYear() - new Date(r.dob).getFullYear() : "N/A"} | Risk: ${r.risk_level} | Home: ${r.home_id || "N/A"} | Service: ${r.service_type || "N/A"}`
).join("\n") || "No residents."}
...
INCIDENTS (last 20):
${incidents.slice(-20).map(i =>
  `- ${i.description || i.incident_type} | Date: ${i.incident_datetime?.split("T")[0] || "N/A"} | Status: ${i.status} | Home: ${i.home_id || "N/A"}`
).join("\n") || "No incidents."}
...
export default function HRAIAssistant({ staff = [], residents = [], homes = [], incidents = [] }) {
  const [open, setOpen] = useState(false);
  const [messages, setMessages] = useState([
    { role: "assistant", content: "Hi! I'm your AI Assistant. I have full app access across HR, residents, homes, incidents, and compliance. How can I help?" }
  ]);
  ...
  const send = async (text) => {
    ...
    setLoading(true);
    try {
      const context = buildContext({ staff, residents, homes, incidents, supervisions, training, balances, leaves, disciplinary, quizResults, policyAssignments, expenses, wellbeing });
      const history = messages.slice(-10).map(m => `${m.role === "user" ? "User" : "Assistant"}: ${m.content}`).join("\n");
      const prompt = `You are an expert assistant for a UK children's care organisation. 
You have full app-wide access to homes, residents, staff, HR, compliance, and incident data:
