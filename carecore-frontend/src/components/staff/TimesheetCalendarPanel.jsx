...
export default function TimesheetCalendarPanel({ timesheet, attendanceLogs, onClose, onUpdate }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-end bg-black/40" onClick={onClose}>
      ...
      >
        ...
        {/* Sync button */}
        {timesheet.status === "draft" && onUpdate && (
          <div className="px-6 py-4 border-t border-border mt-auto bg-card sticky bottom-0">
            <button
              ...
              onClick={() => onUpdate(timesheet.id, { 
                status: "submitted",
                total_actual_hours: totalActual > 0 ? parseFloat(totalActual.toFixed(2)) : timesheet.total_actual_hours
              })}
            >
              Submit Timesheet
            </button>
