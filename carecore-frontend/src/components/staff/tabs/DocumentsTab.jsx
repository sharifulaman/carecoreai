...
const DOC_TITLES = [
  { group: "Employee Induction", items: [
    "Confirmation of Staff Form",
    "HM Revenue & Customs",
    "Medical Questionnaire/Health Certificate",
...
// Flat list for grouping existing docs
const DOC_TYPES = [
  { value: "employee_induction", label: "Employee Induction" },
  { value: "job_description", label: "Job Description/Interview Questions" },
  { value: "reference", label: "Reference" },
  { value: "offer_letter", label: "Offer Letter" },
  { value: "dbs_certificate", label: "DBS / Police Check Certificate" },
  { value: "contract", label: "Contract" },
  { value: "right_to_work", label: "Identification/Right to Work" },
  { value: "proof_of_address", label: "Proof of Address / Driving Licence Check" },
  { value: "cv", label: "CV / Application for Employment" },
  { value: "qualification", label: "Qualification and Training" },
  { value: "probation_appraisal", label: "Probation, Supervision, Appraisals & Complaints" },
  { value: "other", label: "Other" },
];

// Map title → document_type for auto-categorisation
const TITLE_TO_TYPE = {
  "Confirmation of Staff Form": "employee_induction",
  "HM Revenue & Customs": "employee_induction",
  "Medical Questionnaire/Health Certificate": "employee_induction",
  "Data Protection Form": "employee_induction",
  "National Insurance Number Letter": "employee_induction",
  "Job Description/Interview Questions": "job_description",
  "Reference": "reference",
  "Offer Letter": "offer_letter",
  "DBS/Origin-country Police Check Certificate (Overseas Staff Only)": "dbs_certificate",
  "Contract": "contract",
  "Identification/Right to Work": "right_to_work",
  "Proof of Address / Driving Licence Check": "proof_of_address",
  "Curriculum Vitae (CV) / Application for Employment": "cv",
  "Qualification and Training": "qualification",
  "Probation Period, Supervision, Reward, Yearly Appraisals and Complaints": "probation_appraisal",
};
...
function UploadForm({ staffId, staffName, user, onClose, onSaved }) {
  ...
  const handleTitleChange = (title) => {
    const type = TITLE_TO_TYPE[title] || "other";
    setForm(f => ({ ...f, title, document_type: type }));
  };

  const handleSave = async () => {
    if (!form.title) { toast.error("Please select a document title"); return; }
    ...
    <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-card w-full max-w-md rounded-xl shadow-xl p-6 space-y-4">
        ...
        <div>
          ...
          <Select value={form.title} onValueChange={handleTitleChange}>
            <SelectTrigger className="mt-1"><SelectValue placeholder="Select document title…" /></SelectTrigger>
            <SelectContent>
              {DOC_TITLES.map((section, si) => (
                <div key={si}>
                  {section.group && (
                    <div className="px-2 py-1.5 text-[10px] font-semibold text-muted-foreground uppercase tracking-wide border-t first:border-t-0">
                      {section.group}
                    </div>
                  )}
                  {!section.group && si > 0 && <div className="border-t my-1" />}
        ...
        <div className="flex gap-2 justify-end">
          ...
          <Button size="sm" onClick={handleSave} disabled={uploading || !form.title}>
...
export default function DocumentsTab({ staffId, staffName, user }) {
  ...
  const { canAdd, canDelete } = useModuleActions("staff", { canAdd: true, canDelete: isAdmin });
