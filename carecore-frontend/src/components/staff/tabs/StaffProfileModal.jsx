...
import { X, User, Briefcase, PoundSterling, FileText, Shield, Pencil, Save, Eye, EyeOff, TrendingUp } from "lucide-react";
...
import { ORG_ID, DEPARTMENTS } from "@/lib/roleConfig";
...
const TABS = [
  ...
  { key: "performance", label: "Performance",  icon: TrendingUp, managerOnly: true },
];
...
const EDITABLE_BY_ROLE = {
  admin: ["phone", "dob", "gender", "nationality", "address",
    ...
    "job_title", "department", "contract_type", "start_date", "end_date", "probation_end_date",
...
export default function StaffProfileModal({ member, user, homes = [], allStaff = [], defaultTab = "personal", onClose }) {
  ...
  const { canEdit: moduleCanEdit } = useModuleActions("staff", {
    canEdit: isAdmin || viewerRole === "team_leader"
  });
  const canEdit = moduleCanEdit || (viewerRole === "support_worker" && isSelf);
      ...
      <div className="bg-card w-full max-w-xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex border-b border-border overflow-x-auto">
          {TABS.map(t => {
            ...
            if (t.managerOnly && (isSelf || (!isAdmin && viewerRole !== "team_leader"))) return null;
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-1">
          ...
          {activeTab === "employment" && (
            <>
              ...
              {editMode && canEditField("department") ? (
                <div className="flex items-center justify-between py-2 border-b border-border/50 gap-3">
                ...
                <FieldRow label="Department" fieldKey="department" value={DEPARTMENTS.find(d => d.value === form.department)?.label} editMode={false} canEdit={false} onChange={setField} />
              )}
          ...
          {activeTab === "performance" && (
            <StaffPerformanceTab member={currentMember} user={user} />
          )}
        </div>
