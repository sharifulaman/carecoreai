...
export default function HRPolicyTab() {
  ...
  const { canEdit } = useModuleActions("hr");
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (orgId) {
        await secureGateway.update("Organisation", orgId, { hr_policy: policy });
      } else {
        await secureGateway.create("Organisation", { org_id: ORG_ID, name: "CareCore AI", hr_policy: policy });
      }
    },
    onSuccess: () => {
