...
export default function DisciplinaryTab({ user, staff = [] }) {
  ...
  const { canView, canAdd, canEdit, canDelete } = useModuleActions("hr", { 
    canView: canAccess, canAdd: canAccess, canEdit: canAccess, canDelete: canAccess 
  });
  ...
  if (!canView) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <p className="text-sm text-muted-foreground mt-1">Disciplinary & Grievance records are restricted.</p>
      </div>
    );
  }
        ...
        {canAdd && (
          <div className="flex gap-2">
            {activeTab === "disciplinary" && (
              <Button size="sm" variant="default" className="gap-1" onClick={() => { setEditRecord(null); setShowForm("disciplinary"); }}>
                <Plus className="w-3.5 h-3.5" /> Disciplinary
              </Button>
            )}
            {activeTab === "grievance" && (
              <Button size="sm" variant="default" className="gap-1" onClick={() => { setEditRecord(null); setShowForm("grievance"); }}>
                    ...
                    <td className="px-4 py-3">
                      {(canEdit || canDelete) && (
                        <div className="flex items-center gap-1.5">
                          {canEdit && <button onClick={() => { setEditRecord(r); setShowForm("disciplinary"); }} className="text-muted-foreground hover:text-primary">
                            <Pencil className="w-3.5 h-3.5" />
                          </button>}
                          {canDelete && <button onClick={() => setDeleteId(r.id)} className="text-muted-foreground hover:text-red-500">
              ...
              {grievanceRecords.length === 0 ? (
                ...
                <tr key={r.id} className="border-b border-border last:border-0 hover:bg-muted/20">
                  ...
                  <td className="px-4 py-3">
                    {(canEdit || canDelete) && (
                      <div className="flex items-center gap-1.5">
                        {canEdit && <button onClick={() => { setEditRecord(r); setShowForm("grievance"); }} className="text-muted-foreground hover:text-primary"><Pencil className="w-3.5 h-3.5" /></button>}
                        {canDelete && <button onClick={() => setDeleteId(r.id)} className="text-muted-foreground hover:text-red-500"><Trash2 className="w-3.5 h-3.5" /></button>}
                      </div>
                    )}
                  </td>
                </tr>
