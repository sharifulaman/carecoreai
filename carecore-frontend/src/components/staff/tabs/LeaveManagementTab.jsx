...
function RequestForm({ staff, myProfile, prefillDate, onClose, onSubmit }) {
  const [form, setForm] = useState({ leave_type: "annual_leave", date_from: prefillDate || "", date_to: prefillDate || "", notes: "", staff_id: "" });
  const isAdmin = true;
...
export default function LeaveManagementTab({ user, staff = [], homes = [], staffProfile }) {
  ...
  const { canAdd, canEdit, canApprove, canDelete } = useModuleActions("hr", {
    canAdd: true,
    canEdit: isAdmin || isTL,
    canApprove: isAdmin || isTL,
    canDelete: isAdmin || isTL
  });
  ...
  const submitLeave = useMutation({
    mutationFn: async (data) => {
      const created = await secureGateway.create("LeaveRequest", { ...data, status: "pending" });
      // Create ApprovalWorkflow so it appears in the TL's Approvals page
      const requesterProfile = staff.find(s => s.id === data.staff_id);
      const homeId = requesterProfile?.primary_home_id || requesterProfile?.home_ids?.[0] || "";
      const homeName = homes.find(h => h.id === homeId)?.name || "";
      await secureGateway.create("ApprovalWorkflow", {
        org_id: ORG_ID,
        entity_type: "leave_request",
        entity_id: created.id,
        entity_reference: `Leave — ${data.staff_name || requesterProfile?.full_name || ""} — ${data.date_from} to ${data.date_to} (${data.days_requested || 0} days)`,
        home_id: homeId,
        home_name: homeName,
        submitted_by: data.staff_id,
        submitted_by_name: data.staff_name || requesterProfile?.full_name || "",
        submitted_at: new Date().toISOString(),
        status: "pending_tl",
        current_step: 1,
        priority: data.leave_type === "sick_leave" ? "high" : "normal",
        notes: `${(data.days_requested || 0)} days: ${data.date_from} – ${data.date_to}${data.notes ? ". " + data.notes : ""}`,
      });
      await logAudit({
    ...
    onSuccess: (_, data) => {
      ...
      setShowForm(false);
      toast.success("Leave request submitted for TL approval");
      // Notify TL or admin
      ...
      if (recipient?.user_id) {
        createNotification({
          ...
          body: `${data.staff_name || requester?.full_name || "A staff member"} has requested ${(data.leave_type || "").replace(/_/g, " ")} leave from ${data.date_from} to ${data.date_to} (${data.days_requested} days). Please review in Approvals.`,
          type: "leave_request",
          link: "/approvals",
        });
      }
  ...
  const reviewLeave = useMutation({
    mutationFn: async ({ id, status, reason }) => {
      ...
      await secureGateway.update("LeaveRequest", id, {
        ...
        approved_by_name: myProfile?.full_name || user?.full_name,
      ...
      await logAudit({
        ...
        new_values: status === "approved"
          ? { status: "approved", approved_by: myProfile?.full_name || user?.full_name, ssp_eligible: sspPatch.ssp_eligible, ssp_amount: sspPatch.ssp_amount }
          : { status: "rejected", rejection_reason: reason, approved_by: user?.full_name },
        org_id: ORG_ID,
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/40">
                ...
                {(canEdit || canApprove) && <th className="text-center px-4 py-2.5 text-xs font-medium text-muted-foreground">Actions</th>}
              </tr>
            </thead>
            <tbody>
