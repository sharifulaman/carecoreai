...
export default function TimesheetsTab({ user, staff = [], homes = [], staffProfile }) {
  ...
  const { canAdd, canEdit, canApprove } = useModuleActions("hr", {
    canAdd: isAdmin,
  ...
  const [activeTab, setActiveTab] = useState(() => {
    ...
    return params.get("view") || "timesheets";
  });

  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    if (activeTab && activeTab !== "timesheets") {
  ...
  const createPayPeriod = async () => {
    ...
    if (payPeriods && payPeriods.length > 0) {
      const sorted = [...payPeriods].sort((a, b) => b.period_start.localeCompare(a.period_start));
      targetDate = new Date(sorted[0].period_start);
    ...
    await secureGateway.create("PayPeriod", {
      org_id: ORG_ID,
      label,
      period_start: format(startOfMonth(targetDate), "yyyy-MM-dd"),
      period_end: format(endOfMonth(targetDate), "yyyy-MM-dd"),
      frequency: "monthly",
      status: "open",
    });
  ...
  const generateTimesheets = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/business/timesheet/generate`, {
        ...
        body: JSON.stringify({ pay_period_id: period.id })
      });
      if (!response.ok) {
         let msg = "Failed to generate timesheets";
         try { const body = await response.json(); msg = body.error || msg; } catch(e){}
         throw new Error(msg);
      }
      const res = await response.json();
      toast.success(`Timesheets generated! (${res.data.created} new, ${res.data.updated} updated)`);
      queryClient.invalidateQueries({ queryKey: ["timesheets"] });
    } catch (err) {
      toast.error(err.message || "Failed to generate timesheets");
    } finally {
      setGenerating(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
                  ...
                  <tr key={ts.id} className="border-b border-border last:border-0 hover:bg-muted/20 cursor-pointer" onClick={() => setViewDetail(ts)}>
                    ...
                    <td className="px-4 py-3 text-center" onClick={e => e.stopPropagation()}>
                      <div className="flex items-center justify-center gap-2">
                        ...
                        {canAdd && ts.status === "approved" && !hasPayslip && (
      ...
      {/* Payslips */}
      {activeTab === "payslips" && canEdit && filteredPayslips.length > 0 && (
      ...
      {viewDetail && (
        <TimesheetCalendarPanel
          ...
          onUpdate={(id, data) => updateTimesheet.mutate({ id, data })}
        />
      )}
    </div>
  );
