...
export default function OverridesTab({ staffMember, user, myStaffProfile }) {
  ...
  const { canEdit, canAdd } = useModuleActions("staff", {
    canEdit: user?.role === "admin" || user?.role === "admin_officer",
    canAdd: user?.role === "team_leader" || user?.role === "home_manager"
  });
  const isAdminOrTL = canEdit || canAdd;
  ...
  const isSelf = myStaffProfile && staffMember.id === myStaffProfile.id;
  const canAddOverride = isAdminOrTL || isSelf;
