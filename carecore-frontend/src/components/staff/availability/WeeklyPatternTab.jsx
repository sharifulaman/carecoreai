...
function DayCard({ record, isFixed, onSave, canEdit, isExpanded, onExpand }) {
  ...
  // Update local form state if the database record updates after a refetch
  useEffect(() => {
    setForm({
      is_available: record?.is_available ?? true,
      available_from: record?.available_from ?? "07:00",
  ...
  }, [record?.id, record?.is_available, record?.available_from, record?.available_until, record?.shift_type_pref, record?.notes]);
  ...
  return (
    <div className={`rounded-xl p-3 border transition-all ${form.is_available ? "bg-card border-border" : "bg-muted/30 border-border/50"}`}>
      <div 
        className={`flex items-center justify-between cursor-pointer ${isExpanded ? "mb-2" : ""}`}
        onClick={onExpand}
      >
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            ...
            {!isExpanded && (
              form.is_available ? 
                <span className="text-xs text-muted-foreground bg-muted px-1.5 py-0.5 rounded">{form.available_from} - {form.available_until}</span> : 
                <span className="text-xs text-muted-foreground bg-muted px-1.5 py-0.5 rounded">Off</span>
            )}
          </div>
        </div>
        <div className="flex items-center gap-3">
          {canEdit && (
            <button
              ...
              onClick={(e) => { 
                e.stopPropagation(); 
                set("is_available", !form.is_available); 
            ...
            <div className="space-y-2 mt-3">
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <Input type="time" value={form.available_from} onChange={e => set("available_from", e.target.value)} className="mt-0.5 h-8 text-xs" disabled={!canEdit} />
                </div>
                <div>
                  <Label className="text-xs">Until</Label>
                  <Input type="time" value={form.available_until} onChange={e => set("available_until", e.target.value)} className="mt-0.5 h-8 text-xs" disabled={!canEdit} />
                </div>
              </div>
              <div>
                <Label className="text-xs">Preferred shift this day</Label>
                <Select value={form.shift_type_pref} onValueChange={v => set("shift_type_pref", v)} disabled={!canEdit}>
              ...
              <Input placeholder="Notes (optional)" value={form.notes} onChange={e => set("notes", e.target.value)} className="h-8 text-xs" disabled={!canEdit} />
            </div>
          )}
          {canEdit && (
            <Button size="sm" variant="outline" className="mt-3 h-7 text-xs w-full" onClick={handleSave} disabled={saving}>
              {saving ? "Saving..." : "Save"}
            </Button>
          )}
        </div>
...
export default function WeeklyPatternTab({ staffMember, user }) {
  const queryClient = useQueryClient();
  const [expandedDay, setExpandedDay] = useState(null);

  const { canEdit } = useModuleActions("staff", {
    canEdit: user?.role === "admin" || user?.role === "admin_officer" || user?.role === "team_leader" || user?.role === "home_manager"
  });
  ...
  const handleSave = async (record, formData) => {
    ...
    setExpandedDay(null); // Optional: close on save
  };
      ...
      {DAYS.map(day => {
        ...
        return (
          <DayCard 
            key={day} 
            record={record} 
            isFixed={fixedDays.includes(day)} 
            onSave={handleSave} 
            canEdit={canEdit} 
