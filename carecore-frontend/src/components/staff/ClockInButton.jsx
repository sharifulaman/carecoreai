...
import { Clock, LogIn, LogOut, X, MapPin, CheckCircle2, AlertTriangle, UserCheck, UserMinus } from "lucide-react";
      ...
      <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-sm" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div className="rounded-xl border border-blue-200 bg-blue-50 p-3 space-y-2">
            <div className="flex items-center gap-2 text-xs font-semibold text-blue-700">
              <MapPin className="w-3.5 h-3.5" /> Location Tracking
            </div>
            <p className="text-[11px] text-blue-700 leading-relaxed">
              By checking in, you grant permission to your employer to track your GPS location during your work period. Location data is visible only to authorised managers and is used for staff safety and coordination.
            </p>
            <label className="flex items-start gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={locationConsented}
                onChange={e => setLocationConsented(e.target.checked)}
                className="mt-0.5 accent-blue-600 w-3.5 h-3.5 shrink-0"
              />
              <span className="text-[11px] text-blue-800 font-medium">I agree to location tracking during this shift</span>
            </label>
          </div>
        ...
        <div className="px-5 pb-5 flex gap-2">
          ...
          <Button
            ...
            onClick={() => onConfirm({ homeId: selectedHomeId, notes, locationConsented })}
            disabled={isPending}
          >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-sm" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-[11px] text-amber-700 flex items-start gap-2">
            <MapPin className="w-3.5 h-3.5 mt-0.5 shrink-0" />
            Location tracking will stop automatically when you clock out.
          </div>
        </div>
        <div className="px-5 pb-5 flex gap-2">
...
export default function ClockInButton({ user }) {
  ...
  // --- GPS Tracking Logic ---
  const sendLocationUpdate = (pos) => {
    if (!staffProfile?.id) return;
    const { latitude, longitude, accuracy, heading, speed } = pos.coords;
    base44.functions.invoke("recordEmployeeLocation", {
      org_id: ORG_ID,
      ...
      heading,
      speed,
    }).catch(() => { }); // silent fail — will retry next tick
  };
  ...
  const stopLocationTracking = async () => {
    ...
    // Mark the employee location as inactive in DB
    if (staffProfile?.id) {
      try {
        const locs = await base44.entities.EmployeeLocation.filter({ staff_id: staffProfile.id, is_active: true });
        for (const loc of locs) {
          await base44.entities.EmployeeLocation.update(loc.id, { is_active: false });
        }
      } catch (_) { }
    }
  };

  // Save/update consent record in DB
  const saveConsentRecord = async (consented) => {
    if (!staffProfile?.id) return;
    try {
      ...
      if (existing?.length > 0) {
        await base44.entities.LocationTrackingConsent.update(existing[0].id, {
          consented,
          consented_at: consented ? now : existing[0].consented_at,
  ...
  // If already clocked in when component mounts, resume tracking
  useEffect(() => {
    if (activeLog?.clock_in_time && !trackingIntervalRef.current) {
  ...
  const getGps = (consented) => new Promise((resolve) => {
    if (!consented || !navigator.geolocation) { resolve(null); return; }
    setGpsLoading(true);
  ...
  const clockIn = useMutation({
    mutationFn: async ({ homeId, notes, locationConsented }) => {
      ...
      const gps = await getGps(locationConsented);
      ...
      const locationNote = locationConsented ? "Location tracking: enabled" : "Location tracking: declined";
      const combinedNotes = [notes, lateNote, gpsNote, locationNote].filter(Boolean).join(" | ") || null;
      ...
      // Save consent and start tracking if agreed
      await saveConsentRecord(locationConsented);
  ...
  const clockOut = useMutation({
    mutationFn: async ({ notes } = {}) => {
      ...
      // Stop GPS tracking and revoke session consent
  ...
  const isTracking = !!trackingIntervalRef.current;
      ...
      <>
        ...
        >
          ...
          {isAdminType
            ...
            : <><UserCheck className="w-3.5 h-3.5" /> {elapsed || "Clocked In"}</>
          }
          {isTracking && <MapPin className="w-3 h-3 text-blue-500 ml-0.5" title="Location tracking active" />}
        </button>
      </>
    );
  }
    ...
    <>
      {showModal && (
        <ClockInModal
          ...
          org={org}
          onConfirm={({ homeId, notes, locationConsented }) => clockIn.mutate({ homeId, notes, locationConsented })}
      ...
      >
        ...
        {isAdminType ? <UserMinus className="w-3.5 h-3.5" /> : <><UserMinus className="w-3.5 h-3.5" /> Clock In</>}
      </button>
    </>
  );
}