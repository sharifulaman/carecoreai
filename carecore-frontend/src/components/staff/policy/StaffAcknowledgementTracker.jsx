...
import { Search, Filter, MoreVertical, Bell, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function StaffAcknowledgementTracker({ assignments = [], staff = [], canManage, staffProfile, onRefresh }) {
  ...
  const staffStats = useMemo(() => {
    ...
    assignments.forEach(a => {
      ...
      s.total++;
      if (a.assigned_at && (!s.last_activity || a.assigned_at > s.last_activity)) s.last_activity = a.assigned_at;
      if (a.last_reminder_sent_at && (!s.last_activity || a.last_reminder_sent_at > s.last_activity)) s.last_activity = a.last_reminder_sent_at;
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search staff…" className="pl-8 pr-3 py-1.5 text-xs border border-slate-200 rounded-lg focus:outline-none w-40" />
          </div>
      ...
      {filtered.length > 0 && (
        <div className="flex items-center justify-between px-4 py-2 border-t border-slate-100 text-xs text-slate-500">
          <div>
            Showing {((page - 1) * pageSize) + 1} to {Math.min(page * pageSize, filtered.length)} of {filtered.length} staff
          </div>
          <div className="flex items-center gap-1">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="p-1 border border-slate-200 rounded text-slate-500 hover:bg-slate-50 disabled:opacity-40">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <span className="px-2 font-medium">Page {page} of {totalPages}</span>
            <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="p-1 border border-slate-200 rounded text-slate-500 hover:bg-slate-50 disabled:opacity-40">
