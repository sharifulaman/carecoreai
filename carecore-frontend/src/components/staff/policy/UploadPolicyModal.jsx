      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <label className="block border-2 border-dashed border-slate-200 rounded-xl p-6 text-center cursor-pointer hover:border-teal-400 hover:bg-teal-50/30 transition-colors">
              {file ? (
                <div className="flex items-center justify-between">
                  <span className="text-sm text-slate-700 truncate">{file.name}</span>
                  <span onClick={e => { e.preventDefault(); setFile(null); }} className="text-red-400 hover:text-red-600 ml-2 cursor-pointer"><X className="w-4 h-4" /></span>
                </div>
              ) : (
                <>
