...
import { X, Loader2, UserPlus, Users, Building2, Briefcase, Home, Search } from "lucide-react";
...
export default function AssignPolicyModal({ staffProfile, staff, homes, onClose, onSaved }) {
  ...
  const handleAssign = async () => {
    ...
    if (form.scope === "Multiple Staff" && !form.group_name.trim()) { toast.error("Please enter a group name"); return; }
    ...
    try {
      const batchPayload = {
        ...
        policy_version_number: String(policy.current_version_number || "1.0"),
        assignment_name: `${policy.policy_title || "Policy"} — ${form.scope === "Multiple Staff" ? form.group_name : form.scope}`,
        ...
        reminder_frequency: form.reminder_frequency || "Weekly",
        status: "Active",
        staff_count: targetStaff.length,
        notes: form.notes || "",
      };
      if (form.due_date) batchPayload.due_date = form.due_date;

      // Create batch
      const batch = await base44.entities.HRPolicyAssignmentBatch.create(batchPayload);
      ...
      for (const member of targetStaff) {
        ...
        const assignmentPayload = {
          ...
          assignment_name: batchPayload.assignment_name,
          assignment_scope: batchPayload.assignment_scope,
          policy_id: policy.id,
          policy_title: policy.policy_title || "",
          policy_version_number: String(policy.current_version_number || "1.0"),
          staff_id: member.id,
          ...
          staff_department: member.job_title || member.notes || "",
          staff_home_id: home?.id || "",
          staff_home_name: home?.name || "",
          assigned_by_staff_id: staffProfile?.id || "",
          assigned_by_name: staffProfile?.full_name || staffProfile?.name || "",
          assigned_at: new Date().toISOString(),
          status: "Assigned",
        ...
        if (form.due_date) assignmentPayload.due_date = form.due_date;
        
        await base44.entities.HRPolicyStaffAssignment.create(assignmentPayload);
      }
      ...
      await base44.entities.HRPolicyActivityEvent.create({
        ...
        event_description: `${policy.policy_title || "Policy"} assigned by ${staffProfile?.full_name || "Admin"}`,
        policy_id: policy.id,
        policy_title: policy.policy_title || "",
      ...
      console.error("Policy Assignment Error:", e);
      toast.error(e?.data?.message || e?.message || "Failed to assign policy");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-4">
          ...
          {form.scope === "Multiple Staff" && (
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1">Group Name *</label>
              <input 
                type="text" 
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none mb-4" 
          ...
          {(form.scope === "Individual" || form.scope === "Multiple Staff") && (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="block text-xs font-semibold text-slate-500">Select Staff</label>
          ...
          {form.scope === "Role" && (
            <div>
              ...
              <select 
                ...
                value=""
                onChange={(e) => {
                  if (e.target.value && !form.selected_roles.includes(e.target.value)) {
                    toggleRole(e.target.value);
                  }
                }}
              >
                <option value="">Select a role...</option>
                {ROLES.filter(r => !form.selected_roles.includes(r)).map(r => (
                  <option key={r} value={r}>{ROLE_LABELS[r]}</option>
                ))}
              </select>
              {form.selected_roles.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-3">
                  {form.selected_roles.map(r => (
          ...
          {form.scope === "Home" && (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="block text-xs font-semibold text-slate-500">Select Homes</label>
                <div className="relative">
                  <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" />
                  <input 
                    type="text" 
              ...
              <div className="max-h-40 overflow-y-auto border border-slate-200 rounded-lg divide-y divide-slate-100">
                {homes.filter(h => !homeSearchQuery || h.name?.toLowerCase().includes(homeSearchQuery.toLowerCase())).map(h => (
