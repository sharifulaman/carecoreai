  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button className="p-1.5 rounded-lg hover:bg-slate-100 transition-colors outline-none">
          <MoreVertical className="w-4 h-4 text-slate-400" />
        </button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-48 rounded-xl shadow-lg border border-slate-200 py-1 bg-white">
        <DropdownMenuItem onClick={onView} className="px-3 py-2 cursor-pointer gap-2 text-slate-700">
          <Eye className="w-4 h-4" /> View Policy
        </DropdownMenuItem>
        <DropdownMenuItem className="px-3 py-2 cursor-pointer gap-2 text-slate-700">
          <UserPlus className="w-4 h-4" /> View Assignments
        </DropdownMenuItem>
        {canManage && (
          <>
            <DropdownMenuItem onClick={onAssign} className="px-3 py-2 cursor-pointer gap-2 text-slate-700">
              <UserPlus className="w-4 h-4" /> Assign Policy
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => onArchive(policy)} className="px-3 py-2 cursor-pointer gap-2 text-red-600 focus:text-red-700 focus:bg-red-50">
              <X className="w-4 h-4" /> Archive Policy
            </DropdownMenuItem>
          </>
        )}
      </DropdownMenuContent>
