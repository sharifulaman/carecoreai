...
import { useWorkflowTrigger } from "@/hooks/useWorkflowTrigger"; // For triggering workflows on asset updates
...
export default function HomeAssetsTab({ homeId, homeName,staffProfile }) {
  const qc = useQueryClient();
  const { canAdd, canEdit } = useModuleActions("homes");
  const { triggerWorkflow } = useWorkflowTrigger({ staffProfile }); //for triggering workflows on asset updates
  ...
  const { data: assets = [] } = useQuery({
    ...
    onSuccess: (createdRecord, payload) => {
      qc.invalidateQueries({ queryKey: ["home-assets", homeId] });
      setShowForm(false);
      setForm(EMPTY);
      ...
      triggerWorkflow({
        ...
        entityRef:    createdRecord?.id ? `AST-${createdRecord.id.slice(0, 8)}` : "",
        title:        payload.name,
        description:  payload.notes || `${payload.category} asset added to ${homeName}`,
        homeId:       homeId,
        homeName:     homeName,
        priority:     "routine",
      });
                ...
                <tr key={a.id} className="border-b border-border last:border-0 hover:bg-muted/20">
                  ...
                  <td className="px-4 py-3">
                    {canEdit ? (
                      ...
                      <span className={`text-xs px-2 py-1 rounded-lg capitalize ${CONDITION_COLOR[a.condition]}`}>{a.condition?.replace(/_/g, " ")}</span>
                    )}
                  </td>
