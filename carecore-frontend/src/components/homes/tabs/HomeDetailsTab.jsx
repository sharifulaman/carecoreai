  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-xl border border-border p-5 grid sm:grid-cols-2 gap-x-8 gap-y-5">
        ...
        <FIELD label="Care Model" value={home.care_model ? ({ residential: "Residential", outreach: "Outreach", both: "Both" }[home.care_model] || (typeof home.care_model === "string" ? home.care_model.charAt(0).toUpperCase() + home.care_model.slice(1) : home.care_model)) : "Residential"} />
        <FIELD label="Compliance Framework" value={home.type === '18_plus' ? 'Company Policy' : home.compliance_framework?.toUpperCase()} />
          ...
          <div className="space-y-2">
            {homeResidents.map(r => (
              <div key={r.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full capitalize font-medium ${r.risk_level === "high" ? "bg-red-500/10 text-red-600" :
