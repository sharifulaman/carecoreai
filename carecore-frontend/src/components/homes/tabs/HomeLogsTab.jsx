...
export default function HomeLogsTab({ homeId, homeName, user }) {
  ...
  const { data: homeLogs = [] } = useQuery({
    queryKey: ["home-logs", homeId],
    queryFn: () => base44.entities.HomeLog.filter({ org_id: ORG_ID, home_id: homeId }, "-date", 200),
  });

  const { data: dailyLogs = [] } = useQuery({
    queryKey: ["daily-logs-for-home", homeId],
    queryFn: () => base44.entities.DailyLog.filter({ org_id: ORG_ID, home_id: homeId }, "-date", 200),
  });

  // Merge and normalise both sources into a unified shape
  const logs = [
    ...homeLogs,
    ...dailyLogs.map(dl => ({
      id: `dl-${dl.id}`,
      _source: "daily_log",
      date: dl.date,
      created_date: dl.created_date,
      category: dl.log_type ? dl.log_type.toLowerCase() : "general",
      content: dl.summary || dl.title || dl.details || "",
      flagged: dl.flagged || false,
      flag_reason: JSON.stringify({ log_type: dl.log_type || "Daily Log", tags: dl.tags || [], people_name: dl.worker_name || "" }),
      author_name: dl.worker_name || "",
      shift: dl.shift || "n/a",
      home_id: dl.home_id,
    })),
  ...
  const filtered = logs.filter(log => {
    ...
    if (filterType !== "All" && !log.category?.toLowerCase().includes(filterType.toLowerCase())) return false;
              ...
              return (
                <tr key={log.id} className={`border-b border-border/50 last:border-0 align-top ${idx % 2 !== 0 ? "bg-muted/10" : ""}`}>
                  ...
                  <td className="px-4 py-4 text-xs">
                    ...
                    {log._source === "daily_log" && (
                      <span className="ml-1 px-1.5 py-0.5 rounded-full bg-indigo-500/10 text-indigo-600 font-medium text-[10px]">YP Log</span>
                    )}
                  </td>
