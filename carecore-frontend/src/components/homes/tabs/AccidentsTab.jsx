...
import { Plus, Download, X, ChevronRight, CheckCircle2, UserCheck, AlertCircle } from "lucide-react";
import { toast } from "sonner";
import { ORG_ID } from "@/lib/roleConfig";
import { createNotification } from "@/lib/createNotification";
import { useModuleActions } from "@/lib/PermissionContext";
import { useWorkflowTrigger } from "@/hooks/useWorkflowTrigger"; // Custom hook for triggering workflows after form submission
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 flex flex-col items-center gap-4">
        ...
        <div className="text-center">
          <p className="font-bold text-gray-900 text-base mb-1">{isSuccess ? "Report Submitted" : "Submission Failed"}</p>
          <p className="text-sm text-gray-500">{message}</p>
        </div>
        <button
          onClick={onClose}
          className={`w-full py-2.5 rounded-xl text-sm font-medium text-white ${isSuccess ? "bg-teal-600 hover:bg-teal-700" : "bg-red-500 hover:bg-red-600"}`}
        >
...
export default function AccidentsTab({ homeId, homeName, user, residents = [], staff = [] }) {
  ...
  const { canAdd, canEdit } = useModuleActions("homes");
  const { triggerWorkflow } = useWorkflowTrigger({ staffProfile: user }); // Custom hook for triggering workflows after form submission
  const [showForm, setShowForm] = useState(false);
  const [step, setStep] = useState(1);
  const [form, setForm] = useState({ ...EMPTY });
  const [errors, setErrors] = useState({});
  const [notification, setNotification] = useState(null); // { type: "success"|"error", message }
  const f = (k, v) => { setForm(p => ({ ...p, [k]: v })); setErrors(e => ({ ...e, [k]: undefined })); };
  ...
  const [selectedYp, setSelectedYp] = useState("all");
  ...
  reports.forEach(r => {
    const repId = r.reported_by_id || r.reported_by_name;
    if (repId && !seenReporters.has(repId)) {
      seenReporters.add(repId);
      uniqueReporters.push({
        id: repId,
        name: r.reported_by_name || r.reported_by_id || "Unknown",
      });
    }
  });
  ...
  const create = useMutation({
    ...
    onSuccess: (createdRecord, payload) => {
      qc.invalidateQueries({ queryKey: ["accidents"] });
      setShowForm(false);
      setStep(1);
      setForm({ ...EMPTY });
      setErrors({});
      setNotification({ type: "success", message: "The Accident/Illness report has been successfully submitted." });

       // Push into the maker-checker workflow so it shows under
      // "Submitted by Me" in the Workflow Command Centre.
      triggerWorkflow({
        workflowType: "accident",
        entityId:     createdRecord?.id,
        entityRef:    createdRecord?.id ? `ACC-${createdRecord.id.slice(0, 8)}` : "",
        title:        payload.title,
        description:  payload.description,
        homeId:       homeId,
        homeName:     homeName,
        priority:     payload.hospital_attendance ? "urgent" : "routine",
      });
  ...
  const validateStep = (s) => {
    const errs = {};
    if (s === 1) {
      if (!form.type) errs.type = "Please select an accident/illness type";
      if (!form.title.trim()) errs.title = "Title is required";
      if (!form.start_date) errs.start_date = "Start date is required";
    }
    if (s === 2) {
      if (!form.staff_involved) errs.staff_involved = "Please select staff involved or N/A";
      if (!form.yp_involved) errs.yp_involved = "Please select a YP or N/A";
    }
    if (s === 3) {
  ...
  const filtered = reports.filter(r => {
    ...
    // YP Filter
    if (selectedYp !== "all" && r.resident_id !== selectedYp) return false;

    // Staff Filter
    if (selectedStaff !== "all" && r.staff_involved !== selectedStaff) return false;

    // Reporter Filter
    const repId = r.reported_by_id || r.reported_by_name;
    if (selectedReporter !== "all" && repId !== selectedReporter) return false;
  ...
  const exportToPDF = () => {
    ...
    doc.text("Accidents & Incidents Report", margin, yPos);
    yPos += 7;

    // Subtitle / Meta
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    doc.setTextColor(100, 116, 139); // Muted slate text
    doc.text(`Home: ${homeName}`, margin, yPos);
    yPos += 5;
    doc.text(`Date Range: ${dateFrom || "Start"} to ${dateTo || "Today"}`, margin, yPos);
    yPos += 5;
    doc.text(`Generated: ${new Date().toLocaleDateString("en-GB")} | Total Records: ${filtered.length}`, margin, yPos);
    yPos += 8;
    ...
    // Table headers & rows
    const headers = [["Title", "Date", "Type", "Staff Involved", "YP Involved", "Location", "Details", "Status"]];
    ...
    autoTable(doc, {
      ...
      theme: "grid",
      headStyles: {
        fillColor: [15, 118, 110], // Teal #0F766E
        textColor: 255,
        fontSize: 9,
        fontStyle: "bold",
        halign: "left",
    ...
    doc.save(`Accidents_Report_${homeName}_${new Date().toISOString().split("T")[0]}.pdf`);
  };

  const openForm = () => { setForm({ ...EMPTY }); setStep(1); setErrors({}); setShowForm(true); };
    ...
    <div className="space-y-4 relative">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Select value={selectedYp} onValueChange={setSelectedYp}>
          <SelectTrigger className="w-36 h-8 text-xs border border-border rounded-lg bg-card text-muted-foreground hover:bg-muted transition-colors">
            <SelectValue placeholder="Filter by YPs" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">All YPs</SelectItem>
            {homeResidents.map(r => (
              <SelectItem key={r.id} value={r.id}>{r.display_name || r.name}</SelectItem>
            ))}
          </SelectContent>
        </Select>
        ...
        <Select value={selectedReporter} onValueChange={setSelectedReporter}>
          <SelectTrigger className="w-40 h-8 text-xs border border-border rounded-lg bg-card text-muted-foreground hover:bg-muted transition-colors">
            <SelectValue placeholder="Filter by Reporters" />
          </SelectTrigger>
          <SelectContent>
        ...
        <button
          onClick={exportToPDF}
          className="flex items-center gap-1.5 text-xs text-muted-foreground border border-border rounded-lg px-3 h-8 hover:bg-muted transition-colors"
        >
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-start justify-end bg-black/40" onClick={closeForm}>
          <div className="bg-card w-full max-w-4xl h-full overflow-hidden shadow-2xl border-l border-border flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex flex-1 overflow-hidden">
              ...
              <div className="w-56 border-r border-border py-8 px-6 space-y-2 shrink-0 bg-muted/20">
                {STEPS.map(s => (
                  <button key={s.num} onClick={() => { if (s.num < step) setStep(s.num); }}
                    className={`flex items-center gap-3 w-full text-left px-3 py-2.5 rounded-lg transition-colors ${step === s.num ? "bg-card shadow text-foreground font-medium" : s.num < step ? "text-muted-foreground hover:text-foreground" : "text-muted-foreground/40 cursor-not-allowed"}`}>
              ...
              <div className="flex-1 overflow-y-auto px-8 py-6">
                  ...
                  <div className="space-y-5">
                    ...
                    <div>
                      ...
                      <Select value={form.type} onValueChange={v => f("type", v)}>
                        <SelectTrigger className={`text-sm ${errors.type ? "border-red-400 ring-1 ring-red-400" : ""}`}><SelectValue placeholder="Select type..." /></SelectTrigger>
                        <SelectContent>
                    ...
                    <div>
                      ...
                      <Input placeholder="Enter Title" value={form.title} onChange={e => f("title", e.target.value)}
                        className={`text-sm ${errors.title ? "border-red-400 ring-1 ring-red-400" : ""}`} />
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <input type="date" value={form.start_date} onChange={e => f("start_date", e.target.value)}
                          className={`flex-1 h-9 text-sm px-3 border rounded-md bg-card text-foreground placeholder:text-muted-foreground ${errors.start_date ? "border-red-400 ring-1 ring-red-400" : "border-border"}`} />
                        <span className="text-muted-foreground">→</span>
                        <input type="date" value={form.end_date} onChange={e => f("end_date", e.target.value)}
                          className="flex-1 h-9 text-sm px-3 border border-border rounded-md bg-card text-foreground" />
                      </div>
                ...
                {step === 2 && (
                  <div className="space-y-5">
                    ...
                    <div>
                      <p className="text-sm font-medium mb-2">Staff Involved <span className="text-red-500">*</span></p>
                      <Select value={form.staff_involved} onValueChange={v => f("staff_involved", v)}>
                        <SelectTrigger className={`text-sm ${errors.staff_involved ? "border-red-400 ring-1 ring-red-400" : ""}`}><SelectValue placeholder="Select staff..." /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value="N/A">N/A</SelectItem>
                          {staff.map(s => <SelectItem key={s.id} value={s.full_name}>{s.full_name}</SelectItem>)}
                        </SelectContent>
                      </Select>
                      {errors.staff_involved && <p className="text-xs text-red-500 mt-1">{errors.staff_involved}</p>}
                    </div>

                    <div>
                      <p className="text-sm font-medium mb-2">YP (Young Person) Involved <span className="text-red-500">*</span></p>
                      <Select value={form.yp_involved} onValueChange={v => {
                        ...
                        <SelectTrigger className={`text-sm ${errors.yp_involved ? "border-red-400 ring-1 ring-red-400" : ""}`}><SelectValue placeholder="Select YP..." /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value="N/A">N/A</SelectItem>
                          {homeResidents.map(r => <SelectItem key={r.id} value={r.id}>{r.display_name}</SelectItem>)}
                        </SelectContent>
                      </Select>
                      {errors.yp_involved && <p className="text-xs text-red-500 mt-1">{errors.yp_involved}</p>}
                    </div>
                ...
                {step === 3 && (
                  <div className="space-y-5">
                    ...
                    <div>
                      <p className="text-sm font-medium mb-2">Location <span className="text-red-500">*</span></p>
                      <Input placeholder="Where did this occur?" value={form.location} onChange={e => f("location", e.target.value)}
                        className={`text-sm ${errors.location ? "border-red-400 ring-1 ring-red-400" : ""}`} />
                      {errors.location && <p className="text-xs text-red-500 mt-1">{errors.location}</p>}
                    </div>

                    <div>
                      <p className="text-sm font-medium mb-2">Description <span className="text-red-500">*</span></p>
                      <Textarea rows={5} placeholder="Describe what happened in detail..." value={form.description} onChange={e => f("description", e.target.value)}
                        className={`text-sm resize-none ${errors.description ? "border-red-400 ring-1 ring-red-400" : ""}`} />
