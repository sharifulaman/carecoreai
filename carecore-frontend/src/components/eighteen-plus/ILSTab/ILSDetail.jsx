...
export default function ILSDetail({ resident, home, staff, user }) {
  ...
  const { canAdd, canEdit } = useModuleActions("residents");
  ...
  return (
    <div className="space-y-6 bg-card border border-border rounded-lg p-6">
      ...
      {/* ILS Outcome KPIs */}
      <ILSOutcomeKPIs sessions={sessions} residents={[resident]} />
        ...
        {ILS_DOMAINS.map(domain => {
          ...
          return (
            <DomainCard
              ...
              onUpdateSkill={canEdit ? (skill) => setSkillToUpdate({ domain: domain.name, skill }) : undefined}
            />
          );
        })}
      </div>
