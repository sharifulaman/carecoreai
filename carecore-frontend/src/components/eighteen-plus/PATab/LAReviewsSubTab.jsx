...
export default function LAReviewsSubTab({ residents, homes }) {
  ...
  const { data: reviews = [] } = useQuery({
    queryKey: ["la-reviews"],
    queryFn: () => secureGateway.filter("LAReview", {}, "-review_date", 500),
  });
  ...
  const residentRows = residents.map(r => {
    ...
    return {
      ...
      reviewDate: lastReview ? new Date(lastReview.review_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : "—",
      ...
      nextReview: nextDate ? new Date(nextDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : "—",
      status,
      statusColor,
      lastReview,
    };
