    ...
    <div className="bg-card border border-border rounded-lg p-6 space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {new Date(visit.visit_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })} at {new Date(visit.visit_date).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
          </p>
        </div>
      ...
      <div>
        ...
        <p className="font-medium">
          {new Date(visit.next_visit_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })} • {visit.next_visit_type.replace(/_/g, " ")}
        </p>
      </div>
      ...
      <div className="text-xs text-muted-foreground">
        <p>Recorded by {visit.recorded_by_name} on {new Date(visit.created_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</p>
      </div>
    </div>
  );
}