...
function ResidentEducationCard({ resident, onEdit, canEdit }) {
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-4 space-y-3">
      ...
      {!hasDetails && (
        <p className="text-xs text-muted-foreground italic">No provider details recorded.{canEdit && <> <button className="underline text-primary" onClick={() => onEdit(resident)}>Add details →</button></>}</p>
      )}
    </div>
  );
}
    ...
    <div className="mt-4 space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
        {activeResidents.length === 0 ? (
          ...
          <ResidentEducationCard key={r.id} resident={r} onEdit={setEditingResident} canEdit={canEdit} />
        ))}
      </div>
