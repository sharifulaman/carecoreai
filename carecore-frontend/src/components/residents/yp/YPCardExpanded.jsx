...
import EducationAttendanceModal from "../EducationAttendanceModal";
...
export default function YPCardExpanded({
  ...
  onNavigateSP, onNavigateILS, isAdminOrTL, staff, myStaffProfile, onAddDailyLog, onViewSupportPlan, onViewILSPlan
}) {
  const navigate = useNavigate();
  const [showMoreSummary, setShowMoreSummary] = useState(false);
  const [showAllTimeline, setShowAllTimeline] = useState(false);
  const [showEducationModal, setShowEducationModal] = useState(false);
  const [showNightStayModal, setShowNightStayModal] = useState(false);
  const [showMealIntakeModal, setShowMealIntakeModal] = useState(false);
  const [showEduAttendanceModal, setShowEduAttendanceModal] = useState(false);

  // Lazy-load expanded data
  // MAR Today is disabled for now — no data-entry UI exists for MAREntry anywhere
  // in the frontend yet. Re-enable once an eMAR administration form is built.
  ...
  // Current status comes from the same "Update Status" flow used on the collapsed
  // card badge (YPCardView.jsx), which stores the category in content.status_category.
  const currentStatusFlag = currentStatusLog ? { value: currentStatusLog.content?.status_category } : null;
  ...
  // MAR status — disabled, see medRecords/marEntries comment above.
  ...
  // MAR timeline entries — disabled along with MAR Today, see comment above.
    ...
    <div className="border-t border-border">
      ...
      <div className="px-4 py-3 grid grid-cols-2 sm:grid-cols-4 gap-4">
        <Cell label="Current Status">
          {/* Sourced from the "Update Status" badge on the collapsed card (YPCardView.jsx) */}
          {currentStatusFlag
            ? currentStatusFlag.value === "out"
              ? <RedVal>Out</RedVal>
              : currentStatusFlag.value === "unknown"
              ? <AmberVal>Unknown</AmberVal>
              : <GreenVal>{currentStatusFlag.value?.replace(/_/g, " ") || "In"}</GreenVal>
        ...
        <Cell label="Night Stay">
          <button onClick={() => setShowNightStayModal(true)} className="text-left hover:underline">
            {nightStayFlag
              ...
              : <span className="text-amber-700 flex items-center gap-1"><Plus className="w-3 h-3" /> Add</span>}
          </button>
        </Cell>
        <Cell label="Meal Intake">
          <button onClick={() => setShowMealIntakeModal(true)} className="text-left hover:underline">
            {!mealIntakeFlag
              ? <span className="text-amber-700 flex items-center gap-1"><Plus className="w-3 h-3" /> Add</span>
        ...
        <Cell label="Education">
          {(() => {
            ...
            return (
              <button
                onClick={() => isAdminOrTL && setShowEducationModal(true)}
                className={`text-xs px-2 py-0.5 rounded-full font-medium ${colour} ${isAdminOrTL ? "hover:opacity-80 cursor-pointer" : "cursor-default"}`}
              >
                {label}{isAdminOrTL ? " ✎" : ""}
              </button>
            );
        ...
        <Cell label="Education Attendance">
          <button onClick={() => setShowEduAttendanceModal(true)} className="text-left hover:underline">
            {!eduFlag
              ? <span className="text-amber-700 flex items-center gap-1"><Plus className="w-3 h-3" /> Add</span>
        ...
        {/* MAR Today — disabled, no MAREntry creation UI exists yet. See comment near medRecords/marEntries above. */}
      ...
      {showNightStayModal && (
        <NightStayModal resident={resident} user={myStaffProfile} existingLog={nightStayLog} onClose={() => setShowNightStayModal(false)} />
      )}
      {showMealIntakeModal && (
        <MealIntakeModal resident={resident} user={myStaffProfile} existingLog={mealIntakeLog} onClose={() => setShowMealIntakeModal(false)} />
      )}
      {showEduAttendanceModal && (
        <EducationAttendanceModal resident={resident} user={myStaffProfile} existingLog={eduLog} onClose={() => setShowEduAttendanceModal(false)} />
      )}
    </div>
  );
}