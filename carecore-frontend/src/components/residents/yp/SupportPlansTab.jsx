...
import TherapeuticPlanSection from "../behaviour/TherapeuticPlanSection";
...
export default function SupportPlansTab({
  ...
  const { isReadOnly } = useModuleActions("residents", { canEdit: isAdminOrTL });
  ...
  // Attachments are optional and must not block sign-off/completion, even though
  // its displayed status correctly shows "not_started" with zero uploads.
  const allComplete = Object.entries(sectionStatuses).every(
    ([key, status]) => status === "completed" || key === "11. Attachments"
  );
  ...
  return (
    <div className="mt-4 space-y-4">
      ...
      <AccordionSection
        ...
        status={isSignedOff ? "completed" : allComplete ? "not_started" : "in_progress"}
        expandAll={expandAll}
        locked={false}
      >
        <S12Signoff
