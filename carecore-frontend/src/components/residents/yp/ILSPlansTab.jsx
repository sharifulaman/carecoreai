...
function ILSPlanForm({ resident, home, plan, existingSections, canAdd, canEdit, onClose, myStaffProfile }) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (_, status) => {
      ...
      qc.invalidateQueries({ queryKey: ["ils-sections-all"] });
      toast.success("ILS plan saved");
      // Only the evidence-submission step needs review — activation by the
      // reviewer is itself the approval, not something to re-approve.
      if (status === "draft") {
        triggerWorkflow({
          workflowType: "ils",
          entityId:     plan?.id,
          entityRef:    plan?.id ? `ILS-${plan.id.slice(0, 8)}` : "",
          title:        `ILS progress submitted — ${resident.display_name}`,
          description:  `New ILS evidence submitted for ${resident.display_name}, pending TL review.`,
          homeId:       home?.id,
          homeName:     home?.name || "",
          priority:     "routine",
        });
...
export default function ILSPlansTab({ residents, homes, staff, isAdminOrTL, myStaffProfile, defaultResidentId }) {
  ...
  const { canAdd, canEdit } = useModuleActions("residents", { canEdit: isAdminOrTL, canAdd: isAdminOrTL });
