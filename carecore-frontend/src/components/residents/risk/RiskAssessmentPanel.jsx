  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (created) => {
      ...
      triggerWorkflow({
        ...
        entityRef:    (existing?.id || created?.id) ? `RISK-${(existing?.id || created?.id).slice(0, 8)}` : "",
        title:        `${categoryLabel} — ${resident.display_name}`,
        description:  form.background || "Risk assessment submitted for review.",
        homeId:       resident.home_id,
        homeName:     "",
        priority:     overallRating === "high" ? "urgent" : "routine",
      });
