...
export function getHealthStatus(resident) {
  ...
  const hasGP = !!resident.gp_name?.trim();
  const hasConditions = (resident.medical_conditions || []).length > 0;
  const hasNotes = !!resident.health_notes?.trim();
  // Completed if GP is registered, OR if health conditions/notes have been documented
  if (hasGP || hasConditions || hasNotes) return "completed";
  return "not_started";
}
