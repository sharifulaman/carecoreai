...
export function getEducationStatus(resident) {
  ...
  const status = resident.education_status;
  // NEET or any known status counts as completed — education_provider is not required
  // for residents not in education/training
  if (status && status !== "unknown") return "completed";
...
export default function S4EducationSummary({ resident, onNavigateToTab }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Button size="sm" variant="outline" onClick={() => onNavigateToTab?.("education-records")}>
        Go to Education tab →
      </Button>
    </div>
  );
