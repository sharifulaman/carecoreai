...
import { X, MapPin, User, Clock, Calendar, RefreshCw, Pencil, Users, ClipboardList } from "lucide-react";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { secureGateway } from "@/lib/secureGateway";
import { toast } from "sonner";
import { Button } from "@/components/ui/button";
import AppointmentImpactOutcomeSection from "./AppointmentImpactOutcomeSection";
...
const ATTENDANCE_BADGE = {
  ...
  rearranged: "bg-amber-100 text-amber-700",
};

const ENGAGEMENT_LABELS = {
  engaged_well: "Engaged well",
  partially_engaged: "Partially engaged",
  refused: "Refused",
  anxious: "Anxious",
  required_staff_support: "Required staff support",
  not_applicable: "N/A",
};
...
export default function AppointmentDetailPanel({ appointment: apt, residents, staff, staffProfile, isAdmin, onClose, onUpdate, onEdit }) {
  ...
  const [outcomeForm, setOutcomeForm] = useState({
    attendance_status: apt.attendance_status || "",
    missed_reason: apt.missed_reason || "",
    appointment_outcome: apt.appointment_outcome || "",
    impact_on_young_person: apt.impact_on_young_person || "",
    yp_engagement: apt.yp_engagement || "",
    follow_up_required: apt.follow_up_required || false,
    follow_up_notes: apt.follow_up_notes || "",
    responsible_person_id: apt.responsible_person_id || "",
    responsible_person_name: apt.responsible_person_name || "",
    follow_up_target_date: apt.follow_up_target_date || "",
    next_appointment_date: apt.next_appointment_date || "",
    is_priority: apt.is_priority || false,
    priority_reason: apt.priority_reason || "",
    document_types: apt.document_types || [],
    document_urls: apt.document_urls || [],
    staff_comment: apt.staff_comment || "",
    manager_review_required: apt.manager_review_required || false,
    manager_review_note: apt.manager_review_note || "",
    manager_review_status: apt.manager_review_status || "not_required",
  });
  const [savingOutcome, setSavingOutcome] = useState(false);

  const handleSaveOutcome = async () => {
    if (outcomeForm.attendance_status === "missed" && !outcomeForm.missed_reason?.trim()) {
      toast.error("Please provide a reason for the missed appointment");
      return;
    }
    setSavingOutcome(true);
    const targetId = apt._baseId || apt.id;
    await secureGateway.update("Appointment", targetId, {
      ...outcomeForm,
      outcome_recorded_by_id: staffProfile?.id || null,
      outcome_recorded_by_name: staffProfile?.full_name || "",
      outcome_recorded_at: new Date().toISOString(),
      status: outcomeForm.attendance_status === "attended" ? "completed"
        : outcomeForm.attendance_status === "missed" ? "did_not_attend"
        : outcomeForm.attendance_status === "cancelled" ? "cancelled"
        : apt.status,
    });
  ...
  const { canEdit: permCanEdit } = useModuleActions("residents");
  const canModify = permCanEdit && (isAdmin || apt.organiser_id === staffProfile?.id);
  ...
  const currentTab = apt.status === "cancelled" ? "details" : activeTab;
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-end bg-black/40" onClick={onClose}>
        <div className="bg-card w-full max-w-sm h-full overflow-y-auto shadow-2xl border-l border-border" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex border-b border-border">
            {[
              ["details", "Details"],
              ...(apt.status !== "cancelled" ? [["outcome", "Outcome"]] : [])
            ].map(([key, label]) => (
              <button
                key={key}
                onClick={() => setActiveTab(key)}
                className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors ${
                  currentTab === key ? "border-blue-500 text-blue-600" : "border-transparent text-muted-foreground hover:text-foreground"
                }`}
              >
                {label}
                {key === "outcome" && apt.attendance_status && (
                  <span className={`ml-1.5 text-xs px-1.5 py-0.5 rounded-full font-semibold ${ATTENDANCE_BADGE[apt.attendance_status] || "bg-slate-100 text-slate-600"}`}>
          ...
          {currentTab === "outcome" ? (
            <div className="p-4 space-y-4">
              <AppointmentImpactOutcomeSection
                form={outcomeForm}
