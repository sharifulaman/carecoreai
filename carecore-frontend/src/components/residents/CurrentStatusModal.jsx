...
export default function CurrentStatusModal({ resident, user, existingLog, onClose }) {
  ...
  const [statusCategory, setStatusCategory] = useState(existingLog?.content?.statusCategory || "");
  const [notes, setNotes] = useState(existingLog?.content?.notes || "");
  const [linkedAppointmentId, setLinkedAppointmentId] = useState("");

  // Check if resident is in a 24-hour home
  const { data: home } = useQuery({
    queryKey: ["home-type", resident?.home_id],
    queryFn: async () => {
      if (!resident?.home_id) return null;
      const homes = await secureGateway.filter("Home", { id: resident.home_id });
      return homes?.[0] || null;
    },
  ...
  const handleSave = () => {
    ...
    const payload = {
      ...
      content: { status, datetime, statusCategory, notes },
    };
    if (existingLog) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="bg-card rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-5" onClick={e => e.stopPropagation()}>
        ...
        {/* Status Category - Only for 24hr homes */}
        {is24HourHome && (
          <div>
            <p className="text-sm font-medium mb-2">Status Category <span className="text-red-500">*</span></p>
            <select
            ...
            >
              ...
              <option value="awake_home">Awake at Home</option>
              <option value="education">Education/Training</option>
              <option value="work">Work</option>
              <option value="appointment">Appointment</option>
              <option value="activities">Activities/Leisure</option>
