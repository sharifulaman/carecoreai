...
import MissingOutcomeKPIs from "./MissingOutcomeKPIs.jsx";
...
export default function MissingTab({ residents, homes, staff, user, staffProfile, isAdminOrTL, mfhRecords = [] }) {
   const qc = useQueryClient();
  const [showForm, setShowForm] = useState(false);

  // Fetch Impact/Outcome records for missing episodes
  const { data: outcomeRecords = [] } = useQuery({
    queryKey: ["missing-outcomes"],
    queryFn: () => secureGateway.filter("RecordImpactOutcome", { record_type: "missing_episode" }),
    staleTime: 60 * 1000,
  });
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="border-2 border-red-500 bg-red-500/5 rounded-lg p-4 animate-pulse">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3 className="font-bold text-red-700 text-lg">⚠️ ACTIVE MISSING — {activeRecords.length} {activeRecords.length === 1 ? "child" : "children"}</h3>
      ...
      {/* Outcome KPIs */}
      <MissingOutcomeKPIs outcomes={outcomeRecords} mfhRecords={mfhRecords} />
      ...
      {/* Forms & Panels */}
      {showForm && <MissingReportForm resident={selectedResident} residents={residents} homes={homes} staff={staff} user={user} staffProfile={staffProfile} onClose={() => { setShowForm(false); setSelectedResident(null); }} onSave={() => { qc.invalidateQueries({ queryKey: ["mfh-records"] }); setShowForm(false); }} />}
