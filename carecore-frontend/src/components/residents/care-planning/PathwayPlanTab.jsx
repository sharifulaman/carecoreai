...
export default function PathwayPlanTab({ residents, homes, staff, user }) {
  ...
  const eligibleResidents = useMemo(() => residents.filter(r => calcAge(r.dob) >= 16), [residents]);

  useEffect(() => {
    if (eligibleResidents.length > 0) {
      if (!selectedResident || !eligibleResidents.some(r => r.id === selectedResident)) {
        setSelectedResident(eligibleResidents[0].id);
      }
    } else {
      setSelectedResident(null);
    }
  }, [eligibleResidents, selectedResident]);

  const { data: plans = [] } = useQuery({
    queryKey: ["pathway-plans"],
    queryFn: () => secureGateway.filter("PathwayPlan", { is_deleted: false }, "-created_date", 500),
  });
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {residents.length > 1 && (
          ...
          >
            {eligibleResidents.map(r => <option key={r.id} value={r.id}>{r.display_name}</option>)}
          </select>
        )}
