...
import AnnexAHealthAccessCard from "./AnnexAHealthAccessCard";
...
export default function HealthTab({ residents, user, staff, homes = [], staffProfile }) {
  const qc = useQueryClient();
  const { triggerWorkflow } = useWorkflowTrigger({ staffProfile: staffProfile });
  ...
  const saveSection = async () => {
    ...
    triggerWorkflow({
      workflowType: "wellbeing_health",
      entityId: resident.id,
      entityRef: `HEALTH-${resident.id.slice(0, 8)}`,
      title: `Health update — ${resident.display_name}`,
      description: `Section updated: ${editing}`,
  ...
  const EditBtn = ({ section }) => isReadOnly ? null : (
  ...
  return (
    <div className="mt-4 space-y-4">
      ...
      {/* Annex A Health Access Summary */}
      {resident && <AnnexAHealthAccessCard resident={resident} staff={staff} homes={homes} />}
          ...
          <div className="space-y-2 mb-4">
            {resident.allergies.map((a, i) => (
              <div key={i} className="flex items-start gap-3 p-3 rounded-lg bg-muted/30 border border-border">
                ...
                {!isReadOnly && <button onClick={() => removeAllergy(i)} className="text-muted-foreground hover:text-red-500 shrink-0"><X className="w-4 h-4" /></button>}
              </div>
            ))}
          </div>
        ) : (
          ...
          <div className="space-y-2 mb-4">
            {resident.medical_conditions.map((c, i) => (
              <div key={i} className="flex items-start gap-3 p-3 rounded-lg bg-muted/30 border border-border">
                ...
                {!isReadOnly && <button onClick={() => removeCondition(i)} className="text-muted-foreground hover:text-red-500 shrink-0"><X className="w-4 h-4" /></button>}
              </div>
            ))}
          </div>
        ) : (
