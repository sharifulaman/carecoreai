...
export default function NightStayModal({ resident, user, existingLog, onClose }) {
  ...
  const [socialWorkerNotified, setSocialWorkerNotified] = useState(existingLog?.content?.social_worker_notified ?? null);
  ...
  const handleSave = () => {
    ...
    if (status === "Stayed Out" && socialWorkerNotified === null) {
      toast.error("Please specify whether the social worker was notified");
      return;
    }
    const payload = {
      org_id: ORG_ID,
      ...
      shift: "night",
      flags: ["night_stay"],
      content: { night_stay_status: status, night_stay_date: date, notes, social_worker_notified: socialWorkerNotified },
    };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-card rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-5" onClick={e => e.stopPropagation()}>
        ...
        {status === "Stayed Out" && (
          <div>
            <p className="text-sm font-medium mb-2">Social Worker Notified? <span className="text-red-500">*</span></p>
            <div className="flex rounded-lg border border-border overflow-hidden w-fit">
              {[{ label: "Yes", value: true }, { label: "No", value: false }].map(opt => (
                <button
                  key={opt.label}
                  onClick={() => setSocialWorkerNotified(opt.value)}
                  className={`px-5 py-2 text-sm font-medium transition-colors border-r last:border-r-0 border-border ${
                    socialWorkerNotified === opt.value ? "bg-teal-600 text-white" : "bg-card text-muted-foreground hover:bg-muted"
                  }`}
                >
                  {opt.label}
                </button>
