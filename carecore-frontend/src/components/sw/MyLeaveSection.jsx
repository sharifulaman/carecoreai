...
import SelfCertForm from "../staff/ssp/SelfCertForm";
...
export default function MyLeaveSection({ myProfile, org }) {
  ...
  const { triggerWorkflow } = useWorkflowTrigger({ staffProfile: myProfile });
  ...
  const submitLeave = useMutation({
    ...
    onSuccess: async (createdLeave, variables) => {
      queryClient.invalidateQueries({ queryKey: ["my-leave-requests"] });
      setShowModal(false);
      const leaveTypeLabel = LEAVE_TYPES.find(t => t.value === variables.leave_type)?.label || "Leave";
      const days = variables.days_requested;
      await triggerWorkflow({
        workflowType: "leave_request",
        entityId: createdLeave?.id || "",
        entityRef: createdLeave?.id ? `LR-${createdLeave.id.slice(0, 8).toUpperCase()}` : "",
        title: `${leaveTypeLabel} – ${variables.staff_name} (${days} day${days !== 1 ? "s" : ""})`,
        description: `${leaveTypeLabel} from ${variables.date_from} to ${variables.date_to}${variables.notes ? `. Notes: ${variables.notes}` : ""}`,
        homeId: myProfile?.home_ids?.[0] || "",
        priority: "routine",
      });
    },
  });
                    ...
                    <>
                      <tr key={req.id} className="border-b border-border last:border-0 hover:bg-muted/20">
                        ...
                        <td className="px-4 py-3 text-center text-xs">
                          {req.days_requested || (req.date_from && req.date_to
                            ? Math.max(1, differenceInDays(parseISO(req.date_to), parseISO(req.date_from)) + 1)
                            : 0)}
                        </td>
