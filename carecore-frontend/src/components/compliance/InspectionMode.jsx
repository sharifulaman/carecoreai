...
function RecordsTab({ data }) {
  ...
  const { homes = [], mfhRecords = [], bodyMaps = [], handovers = [], complaints = [], significantEvents = [] } = data;
  const homeMap = Object.fromEntries(homes.map(h => [h.id, h]));
...
function GovernanceTab({ data, onSelectDetail }) {
  const { reg45Reviews = [] } = data;
  return (
    <div className="space-y-6">
      <button onClick={() => onSelectDetail("Regulation 45 Reviews", reg45Reviews.map(r => `${r.home_name} - ${r.review_year}`))} className="bg-purple-50 rounded-lg p-4 text-center hover:bg-purple-100 transition-colors cursor-pointer w-full">
      ...
      <p className="text-sm text-muted-foreground">Regulation 45 quality-of-care reviews. Full records in Compliance Hub.</p>
    </div>
  );
}
