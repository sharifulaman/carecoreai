...
import PlacementLegalDetails from "@/components/residents/PlacementLegalDetails";

const TABS = [
  { key: "overview", label: "Overview" },
  { key: "care-plan", label: "Care Plan" },
  { key: "placement-legal", label: "Placement & Legal" },
...
export default function CareResident() {
  ...
  const isAuthorised = isAdmin || isTeamLeader;
  ...
  const renderTab = () => {
    switch (activeTab) {
      case "placement-legal":
        return (
          <PlacementLegalDetails
            resident={resident}
            staffProfile={staffProfile}
            isAuthorised={isAuthorised}
            onUpdate={() => queryClient.invalidateQueries({ queryKey: ["resident-care", resident_id] })}
          />
        );
