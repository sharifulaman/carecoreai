  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-0 border-b border-border">
        {["appointments", "access", "reviews", "support-notes"].map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-3 text-xs font-medium capitalize border-b-2 transition-colors ${activeTab === tab ? "border-teal-600 text-teal-600" : "border-transparent text-muted-foreground"}`}>
            {tab === "access" ? "Health Access" : tab === "support-notes" ? "Support Notes" : tab}
          </button>
        ))}
      </div>

      <div className="bg-card rounded-xl border border-border p-12 text-center">
        <p className="text-muted-foreground">Health & Wellbeing Support — health access, appointments, and support notes coming soon</p>
      </div>
    </div>
  );
}