...
import {
  ...
  Heart, Users, Pill, Award, Pencil, Check, X, Upload
} from "lucide-react";
...
import YPJourneyTab from "@/components/journey/YPJourneyTab";
import WarningLettersAcknowledgementPanel from "@/components/workspace/WarningLettersAcknowledgementPanel";
...
// ── Main page ─────────────────────────────────────────────────────────────────
function ShortNotesEditor({ resident }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await secureGateway.update("Resident", resident.id, { short_notes: value });
  ...
  if (editing) {
    return (
      <div className="flex items-center gap-2 mt-2">
        <input
          autoFocus
          className="flex-1 text-sm border border-primary/40 rounded-lg px-3 py-1.5 bg-background focus:outline-none focus:ring-1 focus:ring-primary"
          value={value}
          onChange={e => setValue(e.target.value)}
          onKeyDown={e => { if (e.key === "Enter") handleSave(); if (e.key === "Escape") handleCancel(); }}
          placeholder="Add short details about this young person…"
          maxLength={200}
        />
        <button onClick={handleSave} disabled={saving} className="text-green-600 hover:text-green-700 p-1 shrink-0">
          <Check className="w-4 h-4" />
        </button>
        <button onClick={handleCancel} className="text-muted-foreground hover:text-foreground p-1 shrink-0">
          <X className="w-4 h-4" />
        </button>
      </div>
    );
  ...
  return (
    <div
      className="flex items-center gap-2 mt-2 group/notes cursor-pointer"
      onClick={() => setEditing(true)}
      title="Click to edit short details"
    >
      <span className={`text-sm ${value ? "text-slate-500" : "text-slate-300 italic"}`}>
        {value || "Add short details…"}
      </span>
      <Pencil className="w-3.5 h-3.5 text-slate-300 group-hover/notes:text-primary transition-colors shrink-0" />
    </div>
  );
}
...
export default function YoungPersonWorkspace() {
  ...
  const [activeSection, setActiveSection] = useState("overview"); // "overview" | "journey" | "yp-placement-journey"
  ...
  useEffect(() => {
    const handler = (e) => { if (quickActionsRef.current && !quickActionsRef.current.contains(e.target)) setQuickActionsOpen(false); };
  ...
  const handleUploadDoc = async (e) => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["rdocs-ws", residentId] });
      setDocName("");
      setDocType("other");
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-3">
        <button onClick={() => navigate("/residents?tab=yp")} className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-slate-800 transition-colors">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-5">
        <div className="flex items-start gap-5 flex-wrap">
          ...
          <div className="flex-1 min-w-0">
            ...
            <ShortNotesEditor resident={resident} />
          </div>
          <div className="shrink-0 flex gap-2">
            <button
              onClick={() => navigate(`/residents?tab=daily-logs-tab&group=records&yp=${residentId}`)}
            ...
            >
              <FileText className="w-3.5 h-3.5" /> Add Daily Log/Report
            </button>
            <div className="relative" ref={quickActionsRef}>
              <button
                onClick={() => setQuickActionsOpen(o => !o)}
                className="px-4 py-2 text-xs font-semibold rounded-xl bg-teal-600 text-white hover:bg-teal-700 transition-colors flex items-center gap-1.5"
              >
                Quick Actions <ChevronDown className="w-3 h-3" />
              </button>
              {quickActionsOpen && (
                <div className="absolute right-0 top-full mt-1.5 w-56 bg-white rounded-xl border border-slate-200 shadow-lg py-1.5 z-20">
                  {[
                    { icon: AlertTriangle, label: "Log Incident", tab: "incidents" },
                    { icon: MapPin, label: "Report Missing", tab: "missing" },
                    { icon: ShieldAlert, label: "Update Risk Assessment", tab: "risk-assessment" },
                    { icon: Zap, label: "Behaviour Management", tab: "behaviour-management" },
                    { icon: Users, label: "Family Contact", tab: "family-contact" },
                  ].map(({ icon: Icon, label, tab }) => (
                    <button
                      key={tab}
                      onClick={() => { setQuickActionsOpen(false); navigate(`/residents?tab=${tab}&yp=${residentId}`); }}
                      className="w-full flex items-center gap-2.5 px-4 py-2 text-xs font-medium text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-colors text-left"
                    >
      ...
      <div className="bg-white border-b border-slate-200 px-6 flex gap-0">
        ...
        <button
          onClick={() => setActiveSection("yp-placement-journey")}
          className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 transition-colors ${activeSection === "yp-placement-journey" ? "border-teal-500 text-teal-600" : "border-transparent text-slate-400 hover:text-slate-600"}`}
        >
          <Milestone className="w-4 h-4" /> YP Journey at Placement
        </button>
      </div>

      {/* ── YP Journey at Placement ── */}
      {activeSection === "yp-placement-journey" && (
      ...
      {activeSection === "overview" && <div className="px-6 py-5 space-y-5">
        ...
        <div className="flex gap-5">
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              <KPICard icon={AlertTriangle} label="Open Incidents" value={openIncidents} color={openIncidents > 0 ? "red" : "slate"} onClick={() => navigate(`/residents?tab=incidents&yp=${residentId}`)} />
              <KPICard icon={ShieldAlert} label="Active Risks" value={activeRisks} color={activeRisks > 0 ? "amber" : "slate"} onClick={() => navigate(`/residents?tab=risk-assessment&yp=${residentId}`)} />
              <KPICard icon={Calendar} label="Upcoming Appointments" value={upcomingAppts.length} sub={nextAppt ? `Next: ${format(new Date(nextAppt.start_datetime || nextAppt.date || today), "dd MMM yyyy")}` : "None scheduled"} color="teal" onClick={() => navigate(`/residents?tab=appointments&yp=${residentId}`)} />
              <KPICard icon={GraduationCap} label="Education Status" value={resident.education_status ? resident.education_status.replace(/_/g, " ").replace(/\b\w/g, c => c.toUpperCase()) : "Unknown"} color="blue" onClick={() => navigate(`/residents?tab=education-records&yp=${residentId}`)} />
              <KPICard icon={MapPin} label="Missing Episodes" value={missingEpisodes} color={missingEpisodes > 0 ? "red" : "slate"} onClick={() => navigate(`/residents?tab=missing&yp=${residentId}`)} />
              <KPICard icon={Zap} label="Outstanding Actions" value={openComplaints + openIncidents} color={openComplaints + openIncidents > 0 ? "amber" : "slate"} onClick={() => navigate(`/residents?tab=complaints&yp=${residentId}`)} />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                ...
                <ViewAll label="View all care plans" onClick={() => navigate(`/residents?tab=support-plans&yp=${residentId}`)} />
              </div>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                ...
                <ViewAll label="View timeline" onClick={() => setShowDailyLogTimeline(true)} />
              </div>
            </div>
          </div>
          ...
          <div className="w-64 xl:w-72 shrink-0 space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setShowDailyLogTimeline(true)} className="text-xs text-teal-600 font-medium hover:underline">View full timeline →</button>
              </div>
              ...
              <ViewAll label="View full timeline" onClick={() => setShowDailyLogTimeline(true)} />
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
              ...
              <div className="mt-4 pt-4 border-t border-slate-100 space-y-2">
                <p className="text-xs font-semibold text-slate-500">Upload Document</p>
                <select
                  value={docType}
                  onChange={e => setDocType(e.target.value)}
                  className="w-full text-xs border border-slate-200 rounded-lg px-2 py-1.5 bg-white text-slate-700 focus:outline-none focus:ring-1 focus:ring-teal-500"
                >
                ...
                <input
                  ...
                  className="w-full text-xs border border-slate-200 rounded-lg px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-teal-500"
                />
                <label className={`flex items-center justify-center gap-1.5 cursor-pointer px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors text-xs font-medium text-slate-600 ${uploadingDoc ? "opacity-50 cursor-not-allowed" : ""}`}>
                  <Upload className="w-3.5 h-3.5" />
                  {uploadingDoc ? "Uploading..." : "Upload PDF"}
                  <input type="file" accept=".pdf,application/pdf" className="hidden" onChange={handleUploadDoc} disabled={uploadingDoc} />
                </label>
              </div>

              <ViewAll label="View all" onClick={() => navigate(`/residents?tab=support-plans&yp=${residentId}`)} />
            </div>

            {/* Warning Letters */}
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
              <h3 className="font-bold text-slate-800 text-sm mb-3 flex items-center gap-2">
                <AlertTriangle className="w-4 h-4 text-amber-500" /> Warning Letters
              </h3>
              <WarningLettersAcknowledgementPanel resident={resident} staff={staff} />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <MiniTabPanel tabs={["Care Plan", "Risks", "Behaviour", "Incidents", "Missing"]}>
              {(active) => (
                <div>
                  ...
                  <ViewAll label="View all" onClick={() => navigate(`/residents?tab=${
                    { "Care Plan": "support-plans", "Risks": "risk-assessment", "Behaviour": "behaviour-management", "Incidents": "incidents", "Missing": "missing" }[active]
                  }&yp=${residentId}`)} />
                </div>
              )}
            </MiniTabPanel>
            ...
            <MiniTabPanel tabs={["Health", "Therapeutic Plan", "Appointments", "Achievements"]}>
              {(active) => (
                <div>
                  ...
                  <ViewAll label="View all" onClick={() => navigate(`/residents?tab=${
                    { "Health": "health", "Therapeutic Plan": "therapeutic-plan", "Appointments": "appointments", "Achievements": "achievements" }[active]
                  }&yp=${residentId}`)} />
                </div>
              )}
            </MiniTabPanel>
            ...
            <MiniTabPanel tabs={["Education", "Family Contact", "Activities", "Housing", "Finance"]}>
              {(active) => (
                <div>
                  ...
                  <ViewAll label="View all" onClick={() => navigate(`/residents?tab=${
                    { "Education": "education-records", "Family Contact": "family-contact", "Activities": "leisure", "Housing": "housing", "Finance": "finance" }[active]
                  }&yp=${residentId}`)} />
                </div>
              )}
            </MiniTabPanel>
          ...
          <SectionCard title="Records & Compliance">
            <MiniTabPanel tabs={["Daily Logs", "Visit Reports", "Complaints"]}>
              {(active) => (
                <div>
                  {active === "Daily Logs" && (
                    <>
                  ...
                  {active !== "Daily Logs" && (
                    <ViewAll label="View all" onClick={() => navigate(`/residents?tab=${
                      { "Visit Reports": "visit-reports", "Complaints": "complaints" }[active]
                    }&yp=${residentId}`)} />
                  )}
                </div>
              )}
