...
import { User, Palette, Building2, Users, Loader2, Sliders, Plus, Trash2, Edit2, ShieldCheck, ClipboardList, AlertTriangle, DollarSign, UserCog, ShieldAlert } from "lucide-react";
...
export default function Settings() {
  ...
  const isRiskLine = ['risk_officer', 'risk_manager'].includes(role);
  const isHRLine = ['hr_officer', 'hr_manager'].includes(role);
  const isFinanceLine = ['finance_officer', 'finance_manager'].includes(role);
  const isCareLine = ['support_worker', 'team_leader', 'team_manager', 'regional_manager', 'rsm'].includes(role);
  const isSeniorManagement = ['admin', 'rsm', 'regional_manager'].includes(role);

  // Role-gated visibility helpers
  const canSeeOrg       = ["admin", "rsm"].includes(role);
  const canSeeUsers     = ["admin", "rsm", "hr_manager"].includes(role);
  const canSeeKPI       = ["admin", "rsm"].includes(role);
  const canSeeHRPolicy  = ["admin", "rsm", "hr_manager"].includes(role);
  const canSeeAdminCP   = ["admin", "rsm"].includes(role);
  const canSeeFinance   = ["admin", "rsm", "finance_manager"].includes(role);
  const canSeeHRTab     = ["admin", "rsm", "hr_manager"].includes(role);
  const canSeeRisk      = ["admin", "rsm", "risk_manager"].includes(role);
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <Tabs defaultValue="profile">
        <TabsList className="bg-muted rounded-xl flex-wrap h-auto gap-1 p-1">
          ...
          {canSeeOrg      && <TabsTrigger value="org"        className="gap-2 rounded-lg"><Building2 className="w-4 h-4" /> Organisation</TabsTrigger>}
          {canSeeUsers    && <TabsTrigger value="users"      className="gap-2 rounded-lg"><Users className="w-4 h-4" /> Users</TabsTrigger>}
          {canSeeKPI      && <TabsTrigger value="kpi"        className="gap-2 rounded-lg"><Sliders className="w-4 h-4" /> KPI Settings</TabsTrigger>}
          {canSeeHRPolicy && <TabsTrigger value="hr-policy"  className="gap-2 rounded-lg"><ClipboardList className="w-4 h-4" /> HR Policy</TabsTrigger>}
          {canSeeFinance  && <TabsTrigger value="finance"    className="gap-2 rounded-lg"><DollarSign className="w-4 h-4" /> Finance Settings</TabsTrigger>}
          {canSeeHRTab    && <TabsTrigger value="hr"         className="gap-2 rounded-lg"><UserCog className="w-4 h-4" /> HR Settings</TabsTrigger>}
          {canSeeRisk     && <TabsTrigger value="risk"       className="gap-2 rounded-lg"><ShieldAlert className="w-4 h-4" /> Risk Settings</TabsTrigger>}
          {canSeeAdminCP  && <TabsTrigger value="admin"      className="gap-2 rounded-lg"><ShieldCheck className="w-4 h-4" /> Admin Control Panel</TabsTrigger>}
        </TabsList>
