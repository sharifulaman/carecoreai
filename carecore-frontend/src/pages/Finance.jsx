...
const TABS = [
  ...
  { key: "occupancy",   label: "Occupancy & Finance" },
...
const KPICard = ({ icon: IconComp, label, value, sub, color = "text-primary", onClick }) => (
  <div 
    className="bg-card rounded-xl border border-border p-4 cursor-pointer hover:shadow-md hover:border-foreground/20 transition-all"
    onClick={onClick}
  >
...
export default function Finance() {
  ...
  const isAdmin = ['admin', 'rsm', 'regional_manager', 'finance_manager', 'finance_officer'].includes(staffRole);
  const isTeamLeader = ['team_leader', 'team_manager'].includes(staffRole);
  const isSW = staffRole === "support_worker";

  // Finance line role checks
  const isFinanceLine = ["finance_officer", "finance_manager", "admin_manager", "rsm", "admin", "regional_manager"].includes(staffRole);
  const isFinanceManager = ["finance_manager", "admin", "rsm"].includes(staffRole);
  const isFinanceOfficer = staffRole === "finance_officer";
  ...
  const sharedData = {
    ...
    isFinanceLine, isFinanceManager, isFinanceOfficer,
  ...
  return (
    <div className="space-y-0 -mx-3 -mt-4 md:-mx-6 md:-mt-6">
      ...
      <div className="px-3 md:px-6 py-4 md:py-6">
        ...
        {activeTab === "occupancy"  && <OccupancyFinanceTab homes={visibleHomes} staffProfile={staffProfile} />}
