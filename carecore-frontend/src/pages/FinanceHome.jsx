...
export default function FinanceHome() {
  ...
  const { canAdd, canEdit } = useModuleActions("finance");
                  ...
                  return (
                    <div key={r.id} className="flex items-center justify-between py-2 border-b border-border/40 last:border-0 text-sm">
                      ...
                      {fee ? (
                        ...
                        canAdd ? <button className="text-xs text-primary hover:underline" onClick={() => setShowFeeForm(true)}>Set fee</button> : null
                      )}
                    </div>
                  );
                })}
