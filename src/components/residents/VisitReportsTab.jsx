...
export default function VisitReportsTab() {
  ...
  return (
    <div className="space-y-4 mt-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-lg font-semibold">Visit Reports / Daily Logs</h2>
          <p className="text-xs text-muted-foreground">{allReports.length} total reports</p>
        </div>
        <Link to="/visit-reports/new">
          <Button className="gap-2" style={{ background: "linear-gradient(135deg, #4B8BF5, #6aa8ff)" }}>
      ...
      {activeTab === "kw_sessions" && (
        <ReportsTable reports={kwSessions} isLoading={isLoading} emptyMsg="No key worker sessions recorded yet." />
      )}
    </div>
  );
}